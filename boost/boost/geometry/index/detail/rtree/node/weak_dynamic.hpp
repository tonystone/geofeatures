...
public:
    ...
    inline allocators(BOOST_FWD_REF(allocators) a)
        : internal_node_allocator_type(geofeatures_boost::move(a.internal_node_allocator()))
        , leaf_allocator_type(geofeatures_boost::move(a.leaf_allocator()))
    {}

    inline allocators & operator=(BOOST_FWD_REF(allocators) a)
    {
        internal_node_allocator() = ::geofeatures_boost::move(a.internal_node_allocator());
        leaf_allocator() = ::geofeatures_boost::move(a.leaf_allocator());
        return *this;
    }
    ...
    {
        geofeatures_boost::swap(internal_node_allocator(), a.internal_node_allocator());
        geofeatures_boost::swap(leaf_allocator(), a.leaf_allocator());
    }
...
struct create_weak_node
{
    ...
    {
        typedef geofeatures_boost::container::allocator_traits<AllocNode> Al;
        typedef typename Al::pointer P;

        P p = Al::allocate(alloc_node, 1);

        if ( 0 == p )
            throw_runtime_error("geofeatures_boost::geometry::index::rtree node creation failed");

        scoped_deallocator<AllocNode> deallocator(p, alloc_node);

        Al::construct(alloc_node, geofeatures_boost::addressof(*p), alloc_node);
...
struct destroy_weak_node
{
    ...
    static inline void apply(AllocNode & alloc_node, BaseNodePtr n)
    {
        ...
        Al::destroy(alloc_node, geofeatures_boost::addressof(*p));
