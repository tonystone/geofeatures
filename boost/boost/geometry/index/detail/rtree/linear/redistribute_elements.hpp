...
inline R difference_dispatch(T const& from, T const& to, ::geofeatures_boost::mpl::bool_<false> const& /*is_unsigned*/)
{
    return to - from;
}

template <typename R, typename T>
inline R difference_dispatch(T const& from, T const& to, ::geofeatures_boost::mpl::bool_<true> const& /*is_unsigned*/)
{
...
{
    BOOST_MPL_ASSERT_MSG(!geofeatures_boost::is_unsigned<R>::value, RESULT_CANT_BE_UNSIGNED, (R));
...
struct find_greatest_normalized_separation
{
    ...
    typedef typename geofeatures_boost::mpl::if_c<
        geofeatures_boost::is_integral<coordinate_type>::value,
        double,
        coordinate_type
