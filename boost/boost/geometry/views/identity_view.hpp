...
{
    typedef typename geofeatures_boost::range_iterator<Range const>::type const_iterator;
    typedef typename geofeatures_boost::range_iterator<Range>::type iterator;

    explicit inline identity_view(Range& r)
        : m_range(r)
    {}

    inline const_iterator begin() const { return geofeatures_boost::begin(m_range); }
    inline const_iterator end() const { return geofeatures_boost::end(m_range); }

    inline iterator begin() { return geofeatures_boost::begin(m_range); }
    inline iterator end() { return geofeatures_boost::end(m_range); }
private :
    Range& m_range;
};
