...
std::pair<bool, bool> point_multipoint_check(Point const& point, MultiPoint const& multi_point)
{
    ...
    typedef typename geofeatures_boost::range_iterator<MultiPoint const>::type iterator;
    iterator it = geofeatures_boost::begin(multi_point);
    iterator last = geofeatures_boost::end(multi_point);
    for ( ; it != last ; ++it )
    {
...
struct point_multipoint
{
    ...
    {
        if ( geofeatures_boost::empty(multi_point) )
        {
...
struct multipoint_multipoint
{
    ...
    static inline void apply(MultiPoint1 const& multi_point1, MultiPoint2 const& multi_point2, Result & result)
    {
        {
            // TODO: throw on empty input?
            bool empty1 = geofeatures_boost::empty(multi_point1);
            bool empty2 = geofeatures_boost::empty(multi_point2);
            if ( empty1 && empty2 )
            {
                return;
            }
...
// TODO: Also, the geometry with the smaller number of points may be analysed first
        //if ( geofeatures_boost::size(multi_point1) < geofeatures_boost::size(multi_point2) )
                              ...
                              Result & result)
    {
        ...
        std::vector<point_type> points(geofeatures_boost::begin(sorted_mpt), geofeatures_boost::end(sorted_mpt));
        ...
        typedef typename geofeatures_boost::range_iterator<IteratedMultiPoint const>::type iterator;
        for ( iterator it = geofeatures_boost::begin(iterated_mpt) ;
              it != geofeatures_boost::end(iterated_mpt) ; ++it )
        {
            bool ii =
