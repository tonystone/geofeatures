...
struct extreme_points_on_ring
{
    ...
    typedef typename geofeatures_boost::range_iterator<Ring const>::type range_iterator;
    ...
    {
        std::size_t const n = geofeatures_boost::size(ring);
    ...
    static inline void get_intruders(Ring const& ring, CirclingIterator left, CirclingIterator right,
            ...
            Intruders& intruders)
    {
        if (geofeatures_boost::size(extremes) < 3)
        {
            return;
        }
        coordinate_type const min_value = geometry::get<Dimension>(*std::min_element(geofeatures_boost::begin(extremes), geofeatures_boost::end(extremes), compare<Dimension>()));

        // Also select left/right (if Dimension=1)
        coordinate_type const other_min = geometry::get<1 - Dimension>(*std::min_element(geofeatures_boost::begin(extremes), geofeatures_boost::end(extremes), compare<1 - Dimension>()));
        coordinate_type const other_max = geometry::get<1 - Dimension>(*std::max_element(geofeatures_boost::begin(extremes), geofeatures_boost::end(extremes), compare<1 - Dimension>()));
        ...
        while (left != right && defensive_check_index < n)
        {
            ...
            if (coordinate > min_value && other_coordinate > other_min && other_coordinate < other_max)
            {
                ...
                if (first_side != 1 && last_side != 1)
                {
                    ...
                    typename geofeatures_boost::range_value<Intruders>::type intruder;
    ...
    static inline bool right_turn(Ring const& ring, Iterator it)
    {
        typename std::iterator_traits<Iterator>::difference_type const index
            = std::distance(geofeatures_boost::begin(ring), it);
        geometry::ever_circling_range_iterator<Ring const> left(ring);
        geometry::ever_circling_range_iterator<Ring const> right(ring);
        left += index;
        right += index;
...
//std::cout << "Candidate at " << geometry::wkt(*it) << " first=" << first_side << " last=" << last_side << std::endl;
    ...
    static inline bool apply(Ring const& ring, Extremes& extremes, Intruders& intruders)
    {
        ...
        range_iterator max_it = geofeatures_boost::begin(ring);
        compare<Dimension> smaller;
        for (range_iterator it = max_it + 1; it != geofeatures_boost::end(ring); ++it)
        {
        ...
        if (max_it == geofeatures_boost::end(ring))
        {
            return false;
        }

        typename std::iterator_traits<range_iterator>::difference_type const
            index = std::distance(geofeatures_boost::begin(ring), max_it);
...
struct extreme_points<MultiPolygon, Dimension, multi_polygon_tag>
{
    ...
    static inline bool apply(MultiPolygon const& multi, Extremes& extremes, Intruders& intruders)
    {
        ...
        if (geofeatures_boost::size(multi) >= 1)
        {
            return extreme_points
                <
                    typename geofeatures_boost::range_value<MultiPolygon const>::type,
                    Dimension,
                    polygon_tag
                >::apply(*geofeatures_boost::begin(multi), extremes, intruders);
        }
...
inline bool extreme_points(Geometry const& geometry, Extremes& extremes, Intruders& intruders)
{
    ...
    concept::check<typename geofeatures_boost::range_value<Extremes>::type>();
    ...
    concept::check
        <
            typename geofeatures_boost::range_value
                <
                    typename geofeatures_boost::range_value<Intruders>::type
                >::type
            const
        >();
