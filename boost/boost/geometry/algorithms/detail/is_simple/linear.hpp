...
public:
    ...
    {
        BOOST_GEOMETRY_ASSERT(geofeatures_boost::size(m_linestring) > 1);
        return m_is_closed
            && turn.method == overlay::method_none
            && check_segment_indices(turn, geofeatures_boost::size(m_linestring) - 2)
...
{
private:
    typedef typename geofeatures_boost::range_value<MultiLinestring>::type linestring_type;
    ...
    {
        BOOST_GEOMETRY_ASSERT(geofeatures_boost::size(linestring) > 1);
        return
                                           ...
                                           Linestring const& linestring)
    {
        ...
        return
            turn.method == overlay::method_none
            &&
            check_segment_indices(turn, geofeatures_boost::size(linestring) - 2)
            &&
...
public:
    ...
    inline bool apply(Turn const& turn) const
    {
        ...
        return
            ...
            &&
            ( geofeatures_boost::size(ls1) != 2
              || geofeatures_boost::size(ls2) != 2
