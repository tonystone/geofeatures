...
struct has_spikes
{
    ...
    static inline bool apply(Range const& range, VisitPolicy& visitor)
    {
        ...
        typedef typename geofeatures_boost::range_iterator<view_type const>::type iterator; 

        bool const is_linear
            = geofeatures_boost::is_same<typename tag<Range>::type, linestring_tag>::value;

        view_type const view(range);

        iterator prev = geofeatures_boost::begin(view);

        iterator cur = find_different_from_first(prev, geofeatures_boost::end(view));
        if (cur == geofeatures_boost::end(view))
        {
        ...
        iterator next = find_different_from_first(cur, geofeatures_boost::end(view));
        if (next == geofeatures_boost::end(view))
        {
        ...
        while (next != geofeatures_boost::end(view))
        {
            ...
            prev = cur;
            cur = next;
            next = find_different_from_first(cur, geofeatures_boost::end(view));
        }

        if (geometry::equals(range::front(view), range::back(view)))
        {
            iterator cur = geofeatures_boost::begin(view);
            typename geofeatures_boost::range_reverse_iterator
                <
                    view_type const
                >::type prev = find_different_from_first(geofeatures_boost::rbegin(view),
                                                         geofeatures_boost::rend(view));
