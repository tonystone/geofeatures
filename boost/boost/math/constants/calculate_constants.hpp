...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost{ namespace math{ namespace constants{ namespace detail{
...
inline T constant_pi<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
   ...
   T lim;
   lim = geofeatures_boost::math::tools::epsilon<T>();
   ...
   do
   {
      ...
      B = ldexp(B, 1);
      result = A - B;
      bool neg = geofeatures_boost::math::sign(result) < 0;
      if(neg)
         result = -result;
      if(result <= lim)
         break;
...
{ // http://oeis.org/A006752/constant
 ...
 //"149374281672134266498119621763019776254769479356512926115106248574");
  ...
  // 9.159655941772190150546035149323841107, 74149374281672134266498119621763019776254769479356512926115106248574422619, -01);
   ...
   return geofeatures_boost::math::constants::pi<T, geofeatures_boost::math::policies::policy<> >()
      * log(2 + geofeatures_boost::math::constants::root_three<T, geofeatures_boost::math::policies::policy<> >())
       / 8
      + 3 * sum / 8;
}
...
T khinchin(int digits)
{
   ...
   return exp(sum / geofeatures_boost::math::constants::ln_two<T, geofeatures_boost::math::policies::policy<> >());
}
...
T zeta_series_derivative_2(unsigned digits)
{
   ...
   geofeatures_boost::math::itrunc((std::numeric_limits<T>::digits10 + 1) * 1.3);
...
inline T zeta_series_derivative_lead_2()
{
   ...
   return -2 * geofeatures_boost::math::constants::ln_two<T>();
}
...
inline T constant_glaisher<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
   ...
   v *= 6;
   v /= geofeatures_boost::math::constants::pi<T, forwarding_policy>() * geofeatures_boost::math::constants::pi<T, forwarding_policy>();
   v -= geofeatures_boost::math::constants::euler<T, forwarding_policy>();
   v -= log(2 * geofeatures_boost::math::constants::pi<T, forwarding_policy>());
   v /= -12;
   return exp(v);
