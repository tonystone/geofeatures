...
#include <boost/pending/property.hpp> // for geofeatures_boost::lookup_one_property
#include <boost/pending/container_traits.hpp>
#include <boost/throw_exception.hpp>
#include <boost/tuple/tuple.hpp> // for geofeatures_boost::make_tuple
...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost { namespace graph {
...
public:
  ...
  {
      geofeatures_boost::throw_exception(std::runtime_error("add_vertex: "
      ...
      typename named_vertices_type::ctor_args_list(
        geofeatures_boost::make_tuple(
          geofeatures_boost::make_tuple(
            ...
            geofeatures_boost::hash<vertex_name_type>(),
...
{
  BOOST_STATIC_ASSERT_MSG ((geofeatures_boost::is_base_of<geofeatures_boost::graph_detail::stable_tag, VertexIterStability>::value), "Named graphs cannot use vecS as vertex container and remove vertices; the lack of vertex descriptor stability (which iterator stability is a proxy for) means that the name -> vertex mapping would need to be completely rebuilt after each deletion.  See https://svn.boost.org/trac/boost/ticket/7863 for more information and a test case.");
