   ...
   :  private MemoryAlgorithm
{
   ...
   //!Allocates nbytes bytes. Throws geofeatures_boost::interprocess::bad_alloc
   //!on failure
                           ...
                           size_type &prefer_in_recvd_out_size, T *&reuse)
   {
      T *ret = MemoryAlgorithm::allocation_command
         (command | geofeatures_boost::interprocess::nothrow_allocation, limit_size, prefer_in_recvd_out_size, reuse);
      if(!(command & geofeatures_boost::interprocess::nothrow_allocation) && !ret)
         throw bad_alloc();
      return ret;
   }

   void *raw_allocation_command  (geofeatures_boost::interprocess::allocation_type command,   size_type limit_objects,
                           size_type &prefer_in_recvd_out_size, void *&reuse, size_type sizeof_object = 1)
   {
      void *ret = MemoryAlgorithm::raw_allocation_command
         ( command | geofeatures_boost::interprocess::nothrow_allocation, limit_objects,
   ...
   :  public segment_manager_base<MemoryAlgorithm>
{
   ...
   //!Can throw geofeatures_boost::interprocess::bad_alloc if there is no enough memory.
   ...
   struct allocator
   {
      typedef geofeatures_boost::interprocess::allocator<T, segment_manager> type;
   };
   ...
   struct deleter
   {
      typedef geofeatures_boost::interprocess::deleter<T, segment_manager> type;
   };
   ...
   {
      geofeatures_boost::interprocess::allocation_type type = ctrl_data->alloc_type();
   ...
   scoped_lock<rmutex> priv_get_lock(bool use_lock)
   {
      ...
      return scoped_lock<rmutex>(geofeatures_boost::move(local));
   }
