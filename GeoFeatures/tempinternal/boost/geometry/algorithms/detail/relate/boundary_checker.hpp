...
public:
    boundary_checker(Geometry const& g)
        : has_boundary( geofeatures_boost::size(g) >= 2
    ...
    {
        geofeatures_boost::ignore_unused_variable_warning(pt);
...
public:
    ...
    bool is_endpoint_boundary(point_type const& pt) const
    {
        ...
        size_type multi_count = geofeatures_boost::size(geometry);
        ...
        if ( ! is_filled )
        {
            ...
            typedef typename geofeatures_boost::range_iterator<Geometry const>::type multi_iterator;
            for ( multi_iterator it = geofeatures_boost::begin(geometry) ;
