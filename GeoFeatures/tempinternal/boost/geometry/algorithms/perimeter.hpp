...
struct perimeter<MultiPolygon, multi_polygon_tag> : detail::multi_sum
{
    ...
    static inline return_type apply(MultiPolygon const& multi, Strategy const& strategy)
    {
        return multi_sum::apply
               <
                   return_type,
                   perimeter<typename geofeatures_boost::range_value<MultiPolygon>::type>
               >(multi, strategy);
    }
};
...
struct perimeter<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    ...
    template <typename Strategy>
    struct visitor: geofeatures_boost::static_visitor<result_type>
    {
