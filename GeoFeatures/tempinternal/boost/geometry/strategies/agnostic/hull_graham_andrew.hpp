...
struct get_extremes
{
    ...
    {
        if (geofeatures_boost::size(range) == 0)
        {
            return;
        }
        ...
        RangeIterator left_it = geofeatures_boost::begin(range);
        RangeIterator right_it = geofeatures_boost::begin(range);

        for (RangeIterator it = geofeatures_boost::begin(range) + 1;
            it != geofeatures_boost::end(range);
            ++it)
        {
            if (less(*it, *left_it))
            {
...
struct assign_range
{
    ...
    inline void apply(InputRange const& range)
    {
        ...
        for (RangeIterator it = geofeatures_boost::begin(range);
            it != geofeatures_boost::end(range);
            ++it)
        {
...
{
    typedef typename geofeatures_boost::range_value<Range>::type point_type;
    typedef geometry::less<point_type> comparator;

    std::sort(geofeatures_boost::begin(range), geofeatures_boost::end(range), comparator());
}
...
public:
    ...
    inline void apply(InputGeometry const& geometry, partitions& state) const
    {
        ...
        //std::cout << geofeatures_boost::size(assigner.lower_points) << std::endl;
        //std::cout << geofeatures_boost::size(assigner.upper_points) << std::endl;
...
private:
    ...
    {
        std::copy(geofeatures_boost::begin(first), geofeatures_boost::end(first), out);

        BOOST_GEOMETRY_ASSERT(closed ? !geofeatures_boost::empty(second) : geofeatures_boost::size(second) > 1);
        std::copy(++geofeatures_boost::rbegin(second), // skip the first Point
                  closed ? geofeatures_boost::rend(second) : --geofeatures_boost::rend(second), // skip the last Point if open
                  out);

        typedef typename geofeatures_boost::range_size<container_type>::type size_type;
        size_type const count = geofeatures_boost::size(first) + geofeatures_boost::size(second) - 1;
        ...
        if (count < 4)
        {
            ...
            *out++ = *geofeatures_boost::begin(first);
        }
    }
};
