...
<
    ...
    typename Difference = typename geofeatures_boost::range_difference
        <
...
class polygon_rings_iterator
    : public geofeatures_boost::iterator_facade
        <
        ...
        >
{
    typedef typename geofeatures_boost::range_size
        <
            typename geofeatures_boost::remove_reference
                <
                    typename interior_return_type<Polygon>::type
...
public:
    ...
    polygon_rings_iterator(Polygon& polygon)
        : m_polygon(geofeatures_boost::addressof(polygon))
        , m_index(0)
    {}
        ...
        , m_index(other.m_index)
    {
        static const bool is_convertible
            = geofeatures_boost::is_convertible<OtherPolygon, Polygon>::value;
...
private:
    friend class geofeatures_boost::iterator_core_access;
...
struct num_rings<MultiPolygon, multi_polygon_tag>
{
    static inline std::size_t apply(MultiPolygon const& multipolygon)
    {
        return geometry::num_interior_rings(multipolygon)
            + static_cast<std::size_t>(geofeatures_boost::size(multipolygon));
    }
};
...
class views_container_initializer<MultiPolygon, multi_polygon_tag>
{
    typedef typename geofeatures_boost::mpl::if_
        <
            geofeatures_boost::is_const<MultiPolygon>,
            typename geofeatures_boost::range_value<MultiPolygon>::type const,
            typename geofeatures_boost::range_value<MultiPolygon>::type
        >::type polygon_type;
    ...
    typedef flatten_iterator
        <
            typename geofeatures_boost::range_iterator<MultiPolygon>::type,
            inner_iterator_type,
...
public:
    ...
    static inline void apply(MultiPolygon const& multipolygon,
                             BoundaryView* views)
    {
        rings_iterator_type first(geofeatures_boost::begin(multipolygon),
                                  geofeatures_boost::end(multipolygon));
        rings_iterator_type last(geofeatures_boost::end(multipolygon));
