...
class BOOST_MPI_DECL communicator
{
 public:
   ...
   */
  communicator(const communicator& comm, const geofeatures_boost::mpi::group& subgroup);
   ...
   */
  geofeatures_boost::mpi::group group() const;
...
{
  packed_oarchive oa(*this);
  oa << n << geofeatures_boost::serialization::make_array(values, n);
  send(dest, tag, oa);
}
                              ...
                              mpl::false_) const
{
  ...
  // Deserialize the data in the message
  geofeatures_boost::serialization::array<T> arr(values, count > n? n : count);
  ia >> arr;
...
{
  shared_ptr<packed_oarchive> archive(new packed_oarchive(*this));
  *archive << n << geofeatures_boost::serialization::make_array(values, n);
  request result = isend(dest, tag, *archive);
  result.m_data = archive;
  return result;
}
