...
{    
    ...
    struct sequence_config
    {
        ...
        BOOST_STATIC_CONSTANT(bool, allow_null = geofeatures_boost::is_nullable<T>::value );
    };
    ...
    public:
        ...
        { 
            ...
            BOOST_ASSERT( !::geofeatures_boost::is_null( this->begin() ) );
            return *this->begin(); 
        }
        ...
        reference back()
        {
            ...
            BOOST_ASSERT( !::geofeatures_boost::is_null( --this->end() ) );
            return *--this->end(); 
        }
    ...
    public: // assign, insert, transfer
        ...
        {
            assign( geofeatures_boost::begin(r), geofeatures_boost::end(r ) );
        }
    ...
    public:
        ...
        geofeatures_boost::disable_if< ptr_container_detail::is_pointer_or_integral<Range> >::type
        insert( iterator before, const Range& r )
        {
            insert( before, geofeatures_boost::begin(r), geofeatures_boost::end(r) );
        }
        ...
        BOOST_DEDUCED_TYPENAME geofeatures_boost::disable_if< geofeatures_boost::is_same< Range,
                      BOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator > >::type
        transfer( iterator before, const Range& r, PtrSeqAdapter& from ) // strong
        {
            transfer( before, geofeatures_boost::begin(r), geofeatures_boost::end(r), from );
        }
    ...
    public: // resize
        ...
        void resize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( geofeatures_boost::next( this->begin(), size ), this->end() );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
                    this->push_back( new BOOST_DEDUCED_TYPENAME 
                                     geofeatures_boost::remove_pointer<value_type>::type() ); 
            }
        ...
        void rresize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( this->begin(), 
                             geofeatures_boost::next( this->begin(), old_size - size ) );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
    ...
    public:
        ...
        void unique( iterator first, iterator last, Compare comp )
        {
            ...
            for( ; next != last; ++next )
            {
                BOOST_ASSERT( !::geofeatures_boost::is_null(prev) );
                BOOST_ASSERT( !::geofeatures_boost::is_null(next) );
                if( comp( *prev, *next ) )
                {
