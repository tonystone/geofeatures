...
/// \brief Contains geofeatures_boost::typeindex::ctti_type_index class.
///
/// geofeatures_boost::typeindex::ctti_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
...
inline const detail::ctti_data& ctti_construct() BOOST_NOEXCEPT {
    ...
    return *reinterpret_cast<const detail::ctti_data*>(geofeatures_boost::detail::ctti<T>::n());
}
...
inline ctti_type_index ctti_type_index::type_id() BOOST_NOEXCEPT {
    ...
    typedef BOOST_DEDUCED_TYPENAME geofeatures_boost::remove_cv<no_ref_t>::type no_cvr_t;
...
inline std::size_t ctti_type_index::hash_code() const BOOST_NOEXCEPT {
    return geofeatures_boost::hash_range(raw_name(), raw_name() + get_raw_name_length());
}
