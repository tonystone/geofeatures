...
# else
                                      ...
                                      std::size_t pos, std::size_t size)
    {
        if(exceptions & io::bad_format_string_bit)
            geofeatures_boost::throw_exception(io::bad_format_string(pos, size) );
    }
                                 ...
                                 unsigned char exceptions) 
    {
        ...
        using namespace geofeatures_boost::io;
        typename String::size_type i1=0;
        int num_items=0;
        while( (i1=buf.find(arg_mark,i1)) != String::npos ) {
            if( i1+1 >= buf.size() ) {
                if(exceptions & bad_format_string_bit)
                    geofeatures_boost::throw_exception(bad_format_string(i1, buf.size() )); // must not end in ".. %"
                else {
                  ++num_items;
                  break;
                }
