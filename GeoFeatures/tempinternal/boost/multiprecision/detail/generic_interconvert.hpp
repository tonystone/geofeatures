...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost{ namespace multiprecision{ namespace detail{
...
template <class To, class B, ::geofeatures_boost::multiprecision::expression_template_option et>
...
#pragma warning(disable:4127)
   ...
   static const int shift = std::numeric_limits<geofeatures_boost::intmax_t>::digits - 1;
   ...
   while(!eval_is_zero(f))
   {
      ...
      typename geofeatures_boost::multiprecision::detail::canonical<geofeatures_boost::intmax_t, To>::type ll;
   ...
   generic_convert_rational_to_float_imp(To& result, Integer& num, Integer& denom, const mpl::false_&)
{
   ...
   if(shift > 0)
      ...
      denom <<= geofeatures_boost::multiprecision::detail::unsigned_abs(shift);
   Integer q, r;
...
void generic_interconvert_float2rational(To& to, const From& from, const mpl::int_<2>& /*radix*/)
{
   ...
   static const int shift = std::numeric_limits<geofeatures_boost::long_long_type>::digits;
   typename From::exponent_type e;
   ...
   while(val)
   {
      val = ldexp(val, shift);
      e -= shift;
      geofeatures_boost::long_long_type ll = geofeatures_boost::math::lltrunc(val);
      val -= ll;
      num <<= shift;
      num += ll;
   }
