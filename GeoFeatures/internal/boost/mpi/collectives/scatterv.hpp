...
{
  if (comm.rank() == root)
    ::geofeatures_boost::mpi::scatterv(comm, &in_values[0], sizes, displs,
                           out_values, out_size, root);
  else
    ::geofeatures_boost::mpi::scatterv(comm, static_cast<const T*>(0), sizes, displs,
                           out_values, out_size, root);
}
         ...
         const std::vector<int>& sizes, T* out_values, int root)
{
  ...
  ::geofeatures_boost::mpi::scatterv(comm, in_values, sizes, displs, out_values,
                         sizes[myrank], root);
}
...
scatterv(const communicator& comm, const std::vector<T>& in_values,
         const std::vector<int>& sizes, T* out_values, int root)
{
  ::geofeatures_boost::mpi::scatterv(comm, &in_values[0], sizes, out_values, root);
}
