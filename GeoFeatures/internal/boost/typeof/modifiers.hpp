...
#define BOOST_TYPEOF_modifier_support(ID, Fun)\
    ...
    {\
        typedef\
            typename geofeatures_boost::type_of::encode_type<\
            typename geofeatures_boost::type_of::push_back<\
            V\
            , geofeatures_boost::mpl::size_t<ID> >::type\
            , T>::type\
            type;\
    };\
    template<class Iter> struct decode_type_impl<geofeatures_boost::mpl::size_t<ID>, Iter>\
    {\
        typedef geofeatures_boost::type_of::decode_type<Iter> d1;\
...
#define BOOST_TYPEOF_array_support(ID, Qualifier)\
    ...
    {\
        typedef\
            ...
            V\
            , geofeatures_boost::mpl::size_t<ID> >::type\
            , geofeatures_boost::mpl::size_t<N> >::type\
            , T>::type\
        type;\
    };\
    template<class Iter>\
    struct decode_type_impl<geofeatures_boost::mpl::size_t<ID>, Iter>\
    {\
        enum{n = Iter::type::value};\
        typedef geofeatures_boost::type_of::decode_type<typename Iter::next> d;\
