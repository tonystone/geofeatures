...
{
    std::size_t const n = geofeatures_boost::size(range);
    if (n > 1)
    {
        return true;
    }
    ...
    else // if (n == 1)
    {
        ...
        geometry::convert(*geofeatures_boost::begin(range), centroid);
        return false;
    }
...
struct centroid_range
{
                             ...
                             Strategy const& strategy)
    {
        if (range_ok(range, centroid))
        {
            ...
            translating_transformer<Range> transformer(*geofeatures_boost::begin(range));
                             ...
                             Strategy const& strategy)
    {
        if (range_ok(exterior_ring(poly), centroid))
        {
            ...
            translating_transformer<Polygon>
                transformer(*geofeatures_boost::begin(exterior_ring(poly)));
...
struct centroid<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    template <typename Point, typename Strategy>
    ...
    apply(geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const& geometry,
          ...
          Strategy const& strategy)
    {
        geofeatures_boost::apply_visitor(visitor<Point, Strategy>(out, strategy), geometry);
    }
};
