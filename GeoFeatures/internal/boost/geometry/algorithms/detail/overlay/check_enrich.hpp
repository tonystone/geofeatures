...
inline bool check_graph(TurnPoints& turn_points, operation_type for_operation)
{
    ...
    int cycle = 0;
    for (typename geofeatures_boost::range_iterator<std::vector<meta_turn<turn_point_type> > > ::type
            it = geofeatures_boost::begin(meta_turns);
         it != geofeatures_boost::end(meta_turns);
         ++it)
    {
