                ...
                LineString const& linestring, Polygon const& polygon)
{
    ...
    detail::point_on_border::midpoint_helper
        ...
        >::apply(point_in_between, *(::geofeatures_boost::begin(linestring) + op.seg_id.segment_index), turn.point);
...
struct action_selector<overlay_intersection, RemoveSpikes>
{
                ...
                OutputIterator& out)
    {
        ...
        detail::overlay::append_no_duplicates(current_piece, point);
        if (::geofeatures_boost::size(current_piece) > 1)
        {
            *out++ = current_piece;
        }
...
public :
                ...
                OutputIterator out)
    {
        ...
        std::sort(geofeatures_boost::begin(turns), geofeatures_boost::end(turns), sort_on_segment<turn_type>());
        ...
        bool entered = false;
        bool first = true;
        for (turn_iterator it = geofeatures_boost::begin(turns); it != geofeatures_boost::end(turns); ++it)
        {
            turn_operation_iterator_type iit = geofeatures_boost::begin(it->operations);
        ...
        if (action::is_entered(entered))
        {
            detail::copy_segments::copy_segments_linestring
                ...
                >::apply(linestring,
                         current_segment_id,
                         static_cast<signed_size_type>(geofeatures_boost::size(linestring) - 1),
                         robust_policy,
                         current_piece);
        }
