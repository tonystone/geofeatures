...
{
    typedef typename geofeatures_boost::range_value<Collection>::type item_type;
    typedef typename item_type::type calculation_type;

    static inline void apply(Collection& collection, Range const& range)
    {
        if (geofeatures_boost::size(range) < 2)
        {
            return;
        }

        typedef typename geofeatures_boost::range_size<Collection>::type collection_size_t;
        collection_size_t c_old_size = geofeatures_boost::size(collection);

        typedef typename geofeatures_boost::range_iterator<Range const>::type iterator;

        bool first = true;
        iterator it = geofeatures_boost::begin(range);

        for (iterator prev = it++;
            it != geofeatures_boost::end(range);
            prev = it++)
        {
            typename geofeatures_boost::range_value<Collection>::type v;
            ...
            calculation_type magnitude = math::sqrt(
                geofeatures_boost::numeric_cast<calculation_type>(v.dx * v.dx + v.dy * v.dy));
        ...
        collection_size_t collected_count = geofeatures_boost::size(collection) - c_old_size;
        if ( collected_count > 1 )
        {
            typedef typename geofeatures_boost::range_iterator<Collection>::type c_iterator;
...
struct box_collect_vectors
{
    ...
    static inline void apply(Collection& collection, Box const& box)
    {
        ...
        typedef typename geofeatures_boost::range_value<Collection>::type item;
