...
public:
    ...
    {
        std::size_t const count = geofeatures_boost::size(linestring);
...
public:
    ...
    bool operator()(Areal const& areal)
    {
        ...
        point_type dummy;
        bool const ok = geofeatures_boost::geometry::point_on_border(dummy, areal);
...
//       Some benchmarking should probably be done to check if only one std::sort should be used
            ...
            for ( ; it != turns.end() ; )
            {
                ...
                prev_seg_id_ptr = geofeatures_boost::addressof(it->operations[1].seg_id);
    ...
    template <typename TurnIt>
    static void sort_dispatch(TurnIt first, TurnIt last, geofeatures_boost::true_type const& /*is_multi*/)
    {
    ...
    template <typename TurnIt>
    static void sort_dispatch(TurnIt first, TurnIt last, geofeatures_boost::false_type const& /*is_multi*/)
    {
...
// TODO: since we update result for some operations here, we may not do it in the analyser!
        ...
        inline bool apply(Range const& turns)
        {
            ...
            for (iterator it = geofeatures_boost::begin(turns) ; it != geofeatures_boost::end(turns) ; ++it)
            {
...
//       this way only the first enter will be handled
                        if ( !m_interior_detected )
                        {
                            ...
                            m_first_interior_other_id_ptr = geofeatures_boost::addressof(other_id);
                        }
                    }
                }
...
// TODO: calculate_from_inside() is only needed if the current Linestring is not closed
            ...
            m_previous_turn_ptr = geofeatures_boost::addressof(*it);
        ...
        void apply(Result & res,
                   ...
                   BoundaryChecker const& boundary_checker)
        {
            geofeatures_boost::ignore_unused(first, last);
            //BOOST_GEOMETRY_ASSERT( first != last );
                                                 ...
                                                 Turn const& turn)
        {
            ...
            typedef typename geofeatures_boost::range_iterator<range2_type>::type range2_iterator;
            range2_type range2(sub_range(geometry2, turn.operations[other_op_id].seg_id));
            
            BOOST_GEOMETRY_ASSERT(geofeatures_boost::size(range1));
            std::size_t const s2 = geofeatures_boost::size(range2);
            ...
            BOOST_GEOMETRY_ASSERT(p_seg_ij + 1 < geofeatures_boost::size(range1));
...
// It would be good to replace it with some O(1) mechanism
                range2_iterator qk_it = find_next_non_duplicated(geofeatures_boost::begin(range2),
                                                                 range::pos(range2, q_seg_jk),
                                                                 geofeatures_boost::end(range2));
