...
struct compute_pythagoras_box_box
{
    ...
    static inline void apply(Box1 const& box1, Box2 const& box2, T& result)
    {
        T const b1_min_coord =
            geofeatures_boost::numeric_cast<T>(geometry::get<min_corner, I-1>(box1));
        T const b1_max_coord =
            geofeatures_boost::numeric_cast<T>(geometry::get<max_corner, I-1>(box1));

        T const b2_min_coord =
            geofeatures_boost::numeric_cast<T>(geometry::get<min_corner, I-1>(box2));
        T const b2_max_coord =
            geofeatures_boost::numeric_cast<T>(geometry::get<max_corner, I-1>(box2));
...
public :
    ...
    */
    ...
    apply(Box1 const& box1, Box2 const& box2)
    {
        ...
        return math::sqrt
            (
                 geofeatures_boost::numeric_cast<typename calculation_type
                     <
                         Box1, Box2
                     >::type>
                    (
