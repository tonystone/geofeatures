...
public:
  ...
  ~copy_map()
  {
    if(!released){
      for(std::size_t i=0;i<n;++i){
        geofeatures_boost::detail::allocator::destroy(&(spc.data()+i)->second->value());
  ...
  void clone(Node* node)
  {
    ...
    BOOST_TRY{
      geofeatures_boost::detail::allocator::construct(
...
private:
  typedef typename geofeatures_boost::detail::allocator::rebind_to<
    Allocator,Node
  >::type                                               allocator_type;
