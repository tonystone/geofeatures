  ...
  namespace chrono
  {
    ...
    process_real_cpu_clock::time_point process_real_cpu_clock::now(system::error_code & ec)
    {
      ...
      if (c == clock_t(-1)) // error
      {
        ...
        {
          geofeatures_boost::throw_exception(system::system_error(errno, BOOST_CHRONO_SYSTEM_CATEGORY, "chrono::process_real_cpu_clock"));
        } else
        {
    ...
    process_user_cpu_clock::time_point process_user_cpu_clock::now(system::error_code & ec)
    {
      ...
      if (c == clock_t(-1)) // error
      {
        if (BOOST_CHRONO_IS_THROWS(ec))
        {
          geofeatures_boost::throw_exception(system::system_error(errno, BOOST_CHRONO_SYSTEM_CATEGORY, "chrono::process_user_cpu_clock"));
        } else
        {
          ec.assign(errno, BOOST_CHRONO_SYSTEM_CATEGORY);
          return time_point();
    ...
    process_system_cpu_clock::time_point process_system_cpu_clock::now(system::error_code & ec)
    {
      ...
      if (c == clock_t(-1)) // error
      {
        if (BOOST_CHRONO_IS_THROWS(ec))
        {
          geofeatures_boost::throw_exception(system::system_error(errno, BOOST_CHRONO_SYSTEM_CATEGORY, "chrono::process_system_cpu_clock"));
        } else
        {
          ec.assign(errno, BOOST_CHRONO_SYSTEM_CATEGORY);
          return time_point();
    ...
    process_cpu_clock::time_point process_cpu_clock::now(system::error_code & ec)
    {
      ...
      if (c == clock_t(-1)) // error
      {
        if (BOOST_CHRONO_IS_THROWS(ec))
        {
          geofeatures_boost::throw_exception(system::system_error(errno, BOOST_CHRONO_SYSTEM_CATEGORY, "chrono::process_clock"));
        } else
        {
          ec.assign(errno, BOOST_CHRONO_SYSTEM_CATEGORY);
          return time_point();
