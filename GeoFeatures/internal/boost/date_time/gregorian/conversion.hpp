...
namespace gregorian {
  ...
  std::tm to_tm(const date& d)
  {
    if (d.is_special())
    {
        ...
        geofeatures_boost::throw_exception(std::out_of_range(s));
    }

    std::tm datetm;
    std::memset(&datetm, 0, sizeof(datetm));
    geofeatures_boost::gregorian::date::ymd_type ymd = d.year_month_day();
...
} } //namespace geofeatures_boost::gregorian
