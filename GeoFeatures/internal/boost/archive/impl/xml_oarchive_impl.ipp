...
void save_iterator(std::ostream &os, InputIterator begin, InputIterator end){
    typedef geofeatures_boost::archive::iterators::mb_from_wchar<
        geofeatures_boost::archive::iterators::xml_escape<InputIterator>
    > translator;
    std::copy(
        translator(begin),
        translator(end),
        geofeatures_boost::archive::iterators::ostream_iterator<char>(os)
    );
}
...
//  at least one library doesn't typedef value_type for strings
//  so rather than using string directly make a pointer iterator out of it
    typedef geofeatures_boost::archive::iterators::xml_escape<
        const char * 
