...
protected:
    ...
    #if 0
    void save_override(const geofeatures_boost::archive::version_type & t){
        ...
        if(geofeatures_boost::archive::library_version_type(6) < lvt){
            const geofeatures_boost::uint_least16_t x = t;
            * this->This() << x;
        }
        else{
    ...
    void save_override(const geofeatures_boost::serialization::item_version_type & t){
        library_version_type lvt = this->get_library_version();
        if(geofeatures_boost::archive::library_version_type(7) < lvt){
            this->detail_common_oarchive::save_override(t);
        }
    ...
    void save_override(const serialization::collection_size_type & t){
        if (get_library_version() < geofeatures_boost::archive::library_version_type(6)){
            unsigned int x=0;
            * this->This() >> x;
