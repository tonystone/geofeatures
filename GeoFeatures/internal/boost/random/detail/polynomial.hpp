...
public:
    ...
    static void mod_pow_x(geofeatures_boost::uintmax_t exponent, const digit_t * mod, std::size_t mod_bits, digit_t * out)
    {
        const std::size_t bits = std::numeric_limits<digit_t>::digits;
        ...
        geofeatures_boost::uintmax_t i = std::numeric_limits<geofeatures_boost::uintmax_t>::digits - 1;
        while(((geofeatures_boost::uintmax_t(1) << i) & exponent) == 0) {
            --i;
        }
        out[0] = 2;
        std::fill_n(out + 1, n - 1, digit_t(0));
        ...
        while(i--) {
            sqr(out, n);
            m(out, 2 * mod_bits - 1);
            if((geofeatures_boost::uintmax_t(1) << i) & exponent) {
                shift_left(out, n, 1);
...
public:
    ...
    friend polynomial mod_pow_x(geofeatures_boost::uintmax_t exponent, polynomial mod);
private:
...
inline polynomial mod_pow_x(geofeatures_boost::uintmax_t exponent, polynomial mod)
{
    polynomial result;
    mod.normalize();
