...
struct seed_type
{
    typedef typename geofeatures_boost::mpl::if_<geofeatures_boost::is_integral<T>,
        T,
        geofeatures_boost::uint32_t
    >::type type;
};
...
void generate_from_real(Engine& eng, Iter begin, Iter end)
{
    ...
    geofeatures_boost::uint_least32_t saved_bits = 0;
    ...
    while(true) {
        ...
        if(Bits < 32 && available_bits < 32 - remaining_bits) {
            saved_bits |= geofeatures_boost::uint_least32_t(val) << remaining_bits;
            remaining_bits += Bits;
        } else {
            ...
            if(Bits < 32 || remaining_bits != 0) {
                geofeatures_boost::uint_least32_t divisor =
                    (geofeatures_boost::uint_least32_t(1) << (32 - remaining_bits));
                geofeatures_boost::uint_least32_t extra_bits = geofeatures_boost::uint_least32_t(fmod(val, mult32)) & (divisor - 1);
                val = val / divisor;
            ...
            if(Bits >= 32) {
                for(; available_bits >= 32; available_bits -= 32) {
                    geofeatures_boost::uint_least32_t word = geofeatures_boost::uint_least32_t(fmod(val, mult32));
                    val /= mult32;
                    *begin++ = word;
                    if(begin == end) return;
                }
            ...
            saved_bits = static_cast<geofeatures_boost::uint_least32_t>(val);
        }
    }
}
...
void generate_from_int(Engine& eng, Iter begin, Iter end)
{
    ...
    typedef typename geofeatures_boost::make_unsigned<IntType>::type unsigned_type;
    int remaining_bits = 0;
    geofeatures_boost::uint_least32_t saved_bits = 0;
    unsigned_type range = geofeatures_boost::random::detail::subtract<IntType>()((eng.max)(), (eng.min)());
    ...
    while(true) {
        unsigned_type val;
        do {
            val = geofeatures_boost::random::detail::subtract<IntType>()(eng(), (eng.min)());
        } while(limit != range && val > limit);
        val &= mask;
        int available_bits = bits;
        if(available_bits == 32) {
            *begin++ = static_cast<geofeatures_boost::uint_least32_t>(val) & 0xFFFFFFFFu;
            if(begin == end) return;
        } else if(available_bits % 32 == 0) {
            for(int i = 0; i < available_bits / 32; ++i) {
                geofeatures_boost::uint_least32_t word = geofeatures_boost::uint_least32_t(val) & 0xFFFFFFFFu;
            ...
            if(bits < 32 || remaining_bits != 0) {
                geofeatures_boost::uint_least32_t extra_bits = geofeatures_boost::uint_least32_t(val) & ((geofeatures_boost::uint_least32_t(1) << (32 - remaining_bits)) - 1);
...
template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, geofeatures_boost::mpl::true_)
{
    return detail::generate_from_int(eng, first, last);
}

template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, geofeatures_boost::mpl::false_)
{
...
{
    return detail::generate_impl(eng, first, last, geofeatures_boost::is_integral<typename Engine::result_type>());
}
...
{
    static const int log = ::geofeatures_boost::mpl::if_c<(m == 0),
        ::geofeatures_boost::mpl::int_<(::std::numeric_limits<IntType>::digits)>,
        ::geofeatures_boost::static_log2<m> >::type::value;
    static const int k =
        (log + ((~(static_cast<IntType>(2) << (log - 1)) & m)? 32 : 31)) / 32;
    ::geofeatures_boost::uint_least32_t array[log / 32 + 4];
...
IntType get_one_int(Iter& first, Iter last)
{
    ...
    for(int j = 0; j < k; ++j) {
        if(first == last) {
            geofeatures_boost::throw_exception(::std::invalid_argument("Not enough elements in call to seed."));
        }
...
{
    geofeatures_boost::uint_least32_t storage[((w+31)/32) * n];
    ...
    for(std::size_t j = 0; j < n; j++) {
        ...
        x[j] = val & ::geofeatures_boost::low_bits_mask_t<w>::sig_bits;
    }
}

template<int w, std::size_t n, class SeedSeq, class IntType>
inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], geofeatures_boost::mpl::true_)
{
    typedef typename geofeatures_boost::make_unsigned<IntType>::type unsigned_array[n];
    seed_array_int_impl<w>(seq, reinterpret_cast<unsigned_array&>(x));
}

template<int w, std::size_t n, class SeedSeq, class IntType>
inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], geofeatures_boost::mpl::false_)
{
...
{
    seed_array_int_impl<w>(seq, x, geofeatures_boost::is_signed<IntType>());
}
...
inline void fill_array_int_impl(Iter& first, Iter last, IntType (&x)[n], geofeatures_boost::mpl::true_)
{
    typedef typename geofeatures_boost::make_unsigned<IntType>::type unsigned_array[n];
...
template<int w, std::size_t n, class Iter, class IntType>
inline void fill_array_int_impl(Iter& first, Iter last, IntType (&x)[n], geofeatures_boost::mpl::false_)
{
...
{
    fill_array_int_impl<w>(first, last, x, geofeatures_boost::is_signed<IntType>());
}

template<int w, std::size_t n, class RealType>
void seed_array_real_impl(const geofeatures_boost::uint_least32_t* storage, RealType (&x)[n])
{
    geofeatures_boost::uint_least32_t mask = ~((~geofeatures_boost::uint_least32_t(0)) << (w%32));
...
void fill_array_real(Iter& first, Iter last, RealType (&x)[n])
{
    ...
    for(j = 0; j < n; ++j) {
        ...
        for(int k = 0; k < w/32; ++k, ++first) {
            if(first == last) geofeatures_boost::throw_exception(std::invalid_argument("Not enough elements in call to seed."));
            val += *first * mult;
            mult *= two32;
        }
        if(mask != 0) {
