...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost { namespace program_options { 
    ...
    template<class T, class charT>
    void 
    typed_value<T, charT>::notify(const geofeatures_boost::any& value_store) const
    {
        const T* value = geofeatures_boost::any_cast<T>(&value_store);
        if (m_store_to) {
            *m_store_to = *value;
        }
        if (m_notifier) {
    ...
    namespace validators {
            ...
            bool allow_empty = false)
        {
            ...
            if (v.size() > 1)
                geofeatures_boost::throw_exception(validation_error(validation_error::multiple_values_not_allowed));
            else if (v.size() == 1)
                return v.front();
            else if (!allow_empty)
                geofeatures_boost::throw_exception(validation_error(validation_error::at_least_one_value_required));
            return empty;
        }

        /* Throws multiple_occurrences if 'value' is not empty. */
        BOOST_PROGRAM_OPTIONS_DECL void 
        check_first_occurrence(const geofeatures_boost::any& value);
    }
    ...
    */
    template<class T, class charT>
    void validate(geofeatures_boost::any& v, 
                  ...
                  T*, long)
    {
        ...
        try {
            ...
            geofeatures_boost::throw_exception(invalid_option_value(s));
        }
    }

    BOOST_PROGRAM_OPTIONS_DECL void validate(geofeatures_boost::any& v, 
    ...
    {
        if (v.empty()) {
            v = geofeatures_boost::any(std::vector<T>());
        }
        std::vector<T>* tv = geofeatures_boost::any_cast< std::vector<T> >(&v);
        assert(NULL != tv);
        for (unsigned i = 0; i < s.size(); ++i)
        {
            try {
                ...
                geofeatures_boost::any a;
                std::vector<std::basic_string<charT> > cv;
                cv.push_back(s[i]);
                validate(a, cv, (T*)0, 0);                
                tv->push_back(geofeatures_boost::any_cast<T>(a));
            }
            catch(const bad_lexical_cast& /*e*/) {
                geofeatures_boost::throw_exception(invalid_option_value(s[i]));
            }
        }
    }
    ...
    void 
    typed_value<T, charT>::
    xparse(geofeatures_boost::any& value_store, 
    ...
    value()
    {
        ...
        return geofeatures_boost::program_options::value<T>(0);
    }
