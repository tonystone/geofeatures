...
struct with_custodian_and_ward : BasePolicy_
{
    ...
    static bool precall(ArgumentPackage const& args_)
    {
        ...
        if (custodian > arity_ || ward > arity_)
        {
            PyErr_SetString(
                PyExc_IndexError
              , "geofeatures_boost::python::with_custodian_and_ward: argument index out of range"
            );
            return false;
        }
...
struct with_custodian_and_ward_postcall : BasePolicy_
{
    ...
    static PyObject* postcall(ArgumentPackage const& args_, PyObject* result)
    {
        ...
        if ( (std::max)(custodian, ward) > arity_ )
        {
            PyErr_SetString(
                PyExc_IndexError
              , "geofeatures_boost::python::with_custodian_and_ward_postcall: argument index out of range"
            );
            return 0;
        }
