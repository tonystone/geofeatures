...
public:
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        geofeatures_boost::sp_array_constructor_hook( px );
#endif
    }
    ...
    {
#if defined(BOOST_SP_ENABLE_DEBUG_HOOKS)
        geofeatures_boost::sp_array_destructor_hook( px );
#endif
        geofeatures_boost::checked_array_delete( px );
    }
...
template<class T> inline bool operator==( scoped_array<T> const & p, geofeatures_boost::detail::sp_nullptr_t ) BOOST_NOEXCEPT
{
    return p.get() == 0;
}

template<class T> inline bool operator==( geofeatures_boost::detail::sp_nullptr_t, scoped_array<T> const & p ) BOOST_NOEXCEPT
{
    return p.get() == 0;
}
