...
//      great as geofeatures_boost::intmax_t can cause some differences to be
...
# ifndef BOOST_NO_LIMITS_COMPILE_TIME_CONSTANTS
  ...
  // This should probably be rolled into geofeatures_boost::integer_traits one day, but I
      ...
      if_true<(int(x::is_signed)
              && (!int(x::is_bounded)
                 // digits is the number of no-sign bits
                  || (int(x::digits) + 1 >= digit_traits<geofeatures_boost::intmax_t>::digits)))>::template then<
        Integer,
...
#else
      BOOST_STATIC_ASSERT(geofeatures_boost::is_integral<Integer>::value);
