...
struct node_alloc_holder
{
   ...
   typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(geofeatures_boost::container::container_detail::, ICont,
   ...
   typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(geofeatures_boost::container::container_detail::, intrusive_value_compare,
   ...
   typedef container_detail::integral_constant<unsigned,
      geofeatures_boost::container::container_detail::
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x)
      : members_(geofeatures_boost::move(x.node_alloc()))
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x, const value_compare &c)
      : members_(geofeatures_boost::move(x.node_alloc()), c)
   {  this->icont().swap(x.icont());  }
   ...
   NodePtr create_node(Args &&...args)
   {
      ...
      allocator_traits<NodeAlloc>::construct
         ( this->node_alloc()
         , container_detail::addressof(p->m_data), geofeatures_boost::forward<Args>(args)...);
   ...
   NodePtr create_node_from_it(const It &it)
   {
      NodePtr p = this->allocate_one();
      Deallocator node_deallocator(p, this->node_alloc());
      ::geofeatures_boost::container::construct_in_place(this->node_alloc(), container_detail::addressof(p->m_data), it);
      node_deallocator.release();
      //This does not throw
      typedef typename Node::hook_type hook_type;
      ::new(static_cast<hook_type*>(container_detail::to_raw_pointer(p)), boost_container_new_t()) hook_type;
      ...
      if(n){
         ...
         BOOST_TRY{
            ...
            while(n--){
               ...
               ++itbeg;
               //This can throw
               geofeatures_boost::container::construct_in_place(nalloc, container_detail::addressof(p->m_data), beg);
               sdestructor.set(p);
               ++beg;
               //This does not throw
               typedef typename Node::hook_type hook_type;
   ...
   void clear(version_2)
   {
      ...
      //BOOST_STATIC_ASSERT((::geofeatures_boost::has_move_emulation_enabled<typename NodeAlloc::multiallocation_chain>::value == true));
      if(!chain.empty())
   ...
   struct move_cloner
   {
      ...
      {  //Use m_data instead of get_data to allow moving const key in [multi]map
         return m_holder.create_node(::geofeatures_boost::move(other.m_data));
      }
      ...
      :  public NodeAlloc
   {
      ...
      explicit members_holder(BOOST_FWD_REF(ConvertibleToAlloc) c2alloc)
         :  NodeAlloc(geofeatures_boost::forward<ConvertibleToAlloc>(c2alloc))
         , m_icont()
      {}
