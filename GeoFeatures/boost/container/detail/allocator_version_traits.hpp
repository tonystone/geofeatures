...
template<class Allocator, unsigned Version = geofeatures_boost::container::container_detail::version<Allocator>::value>
...
struct allocator_version_traits<Allocator, 1>
{
   ...
   typedef typename geofeatures_boost::intrusive::pointer_traits<pointer>::
   ...
   static void deallocate_individual(Allocator &a, multiallocation_chain &holder)
   {
      ...
      while(n--){
         pointer p = geofeatures_boost::intrusive::pointer_traits<pointer>::pointer_to(*it);
         ++it;
         a.deallocate(p, 1);
      }
   }
