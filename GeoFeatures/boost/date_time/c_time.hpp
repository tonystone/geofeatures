...
namespace date_time {
  ...
  struct c_time {
	...
	#endif
        if (!result)
          geofeatures_boost::throw_exception(std::runtime_error("could not convert calendar time to local time"));
        return result;
      }
      //! requires a pointer to a user created std::tm struct
      inline
	...
	#endif
        if (!result)
          geofeatures_boost::throw_exception(std::runtime_error("could not convert calendar time to UTC time"));
        return result;
      }
