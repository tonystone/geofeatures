...
namespace date_time {
  ...
  */
  ...
  class time_duration : private
      geofeatures_boost::less_than_comparable<T
    , geofeatures_boost::equality_comparable<T
    > >
   ...
   */
  template<class base_duration, geofeatures_boost::int64_t frac_of_second>
  ...
  private:
    ...
    BOOST_STATIC_CONSTANT(geofeatures_boost::int64_t, adjustment_ratio = (traits_type::ticks_per_second >= frac_of_second ? traits_type::ticks_per_second / frac_of_second : frac_of_second / traits_type::ticks_per_second));

  public:
    explicit subsecond_duration(geofeatures_boost::int64_t ss) :
