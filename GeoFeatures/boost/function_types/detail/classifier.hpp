...
template<bits_t Flags, bits_t CCID, std::size_t Arity> struct encode_charr
{
  typedef typename char_array<
    ::geofeatures_boost::function_types::detail::encode_charr_impl<Flags,CCID,Arity>::value 
  >::type type;
};
...
{
  static typename geofeatures_boost::add_reference<T>::type tester;

  BOOST_STATIC_CONSTANT(bits_t,value = (bits_t)sizeof(
    geofeatures_boost::function_types::detail::classifier_impl(& tester) 
  )-1);
};
...
template<typename T> struct classifier
{
  typedef detail::constant<
    ::geofeatures_boost::function_types::detail::decode_bits<
      ::geofeatures_boost::function_types::detail::classifier_bits<T>::value
    >::tag_bits > 
  bits;
