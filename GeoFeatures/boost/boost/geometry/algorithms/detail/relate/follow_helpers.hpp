...
template <std::size_t OpId,
          ...
          bool IsMulti = geofeatures_boost::is_base_of<multi_tag, Tag>::value
>
...
struct for_each_disjoint_geometry_if<OpId, Geometry, Tag, true>
{
                                 ...
                                 Pred & pred)
    {
        ...
        for ( iterator it = geofeatures_boost::begin(geometry) ;
              it != geofeatures_boost::end(geometry) ; ++it )
        {
            bool cont = pred(*it);
            if ( !cont )
                break;
        ...
        return !geofeatures_boost::empty(geometry);
    }
                                 ...
                                 Pred & pred)
    {
        ...
        const std::size_t count = geofeatures_boost::size(geometry);
        geofeatures_boost::ignore_unused_variable_warning(count);
...
public:
    ...
    point_info(Point const& pt, segment_identifier const& sid)
        : sid_ptr(geofeatures_boost::addressof(sid))
        , pt_ptr(geofeatures_boost::addressof(pt))
    {}
...
public:
    ...
    bool update(segment_identifier const& seg_id)
    {
        ...
        m_seg_id_ptr = geofeatures_boost::addressof(seg_id);
        return result;
    }
    ...
    void exit(TurnInfo const& turn, bool exit_per_geometry = true)
    {
        ...
        if ( entry_it != m_other_entry_points.end() )
        {
            ...
            if ( exit_per_geometry || m_other_entry_points.empty() )
            {
                ...
                m_exit_turn_ptr = geofeatures_boost::addressof(turn);
            }
        }
    }
...
{
    geofeatures_boost::ignore_unused_variable_warning(seg_id);
