...
void PBGL_DISTRIB_PMAP::prune_ghost_cells() const
{
  ...
  while (data->ghost_cells->size() > data->max_ghost_cells) {
    ...
    if (data->model & cm_flush) {
      ...
      geofeatures_boost::parallel::detail::untracked_pair<key_type, value_type> const& victim
...
handle_put(int /*source*/, int /*tag*/, 
           const geofeatures_boost::parallel::detail::untracked_pair<key_type, value_type>& req, trigger_receive_context)
{
  using geofeatures_boost::get;
                ...
                trigger_receive_context)
{
  ...
  typedef geofeatures_boost::parallel::detail::untracked_pair<key_type, value_type> key_value;
...
handle_multiget_reply
  (int source, int tag, 
   const std::vector<geofeatures_boost::parallel::detail::untracked_pair<key_type, value_type> >& msg,
   trigger_receive_context)
{
  shared_ptr<data_t> data(data_ptr);
  BOOST_ASSERT(data);
...
{
  using geofeatures_boost::parallel::simple_trigger;
...
PBGL_DISTRIB_PMAP::set_max_ghost_cells(std::size_t max_ghost_cells)
{
  if ((data->model & cm_backward) && max_ghost_cells > 0)
      geofeatures_boost::throw_exception(std::runtime_error("distributed_property_map::set_max_ghost_cells: "
