  ...
  public:
    ...
    void get_trapezoids(cT& container, orientation_2d slicing_orientation) const {
      if(slicing_orientation == VERTICAL) {
        ...
        for(typename cT::iterator itr = result.begin(); itr != result.end(); ++itr) {
          ::geofeatures_boost::polygon::transform(*itr, axis_transformation(axis_transformation::SWAP_XY));
        }
    ...
    transform(const transform_type& tr) {
      ...
      for(std::size_t i = 0 ; i < polys.size(); ++i) {
        ::geofeatures_boost::polygon::transform(polys[i], tr);
        insert(polys[i]);
      }
      unsorted_ = true;
      dirty_ = true;
    ...
    scale_up(typename coordinate_traits<coordinate_type>::unsigned_area_type factor) {
      for(typename value_type::iterator itr = data_.begin(); itr != data_.end(); ++itr) {
        ::geofeatures_boost::polygon::scale_up((*itr).first.first, factor);
        ::geofeatures_boost::polygon::scale_up((*itr).first.second, factor);
      }
      return *this;
    }
    ...
    scale_down(typename coordinate_traits<coordinate_type>::unsigned_area_type factor) {
      for(typename value_type::iterator itr = data_.begin(); itr != data_.end(); ++itr) {
        ...
        ::geofeatures_boost::polygon::scale_down((*itr).first.first, factor);
        ::geofeatures_boost::polygon::scale_down((*itr).first.second, factor);
    ...
    inline polygon_set_data& scale(polygon_set_data& polygon_set,
      ...
      for(typename value_type::iterator itr = begin(); itr != end(); ++itr) {
        bool vb = (*itr).first.first.x() == (*itr).first.second.x();
        ::geofeatures_boost::polygon::scale((*itr).first.first, scaling);
        ::geofeatures_boost::polygon::scale((*itr).first.second, scaling);
        bool va = (*itr).first.first.x() == (*itr).first.second.x();
        if(!vb && va) {
          (*itr).second *= -1;
        }
