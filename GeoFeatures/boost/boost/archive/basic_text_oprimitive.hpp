...
protected:
    ...
    template<class T>
    void save_impl(const T &t, geofeatures_boost::mpl::bool_<false> &){
        if(os.fail())
            geofeatures_boost::serialization::throw_exception(
                archive_exception(archive_exception::output_stream_error)
            );
    ...
    struct is_float {
        typedef typename mpl::bool_< 
            geofeatures_boost::is_floating_point<T>::value 
    ...
    template<class T>
    void save_impl(const T &t, geofeatures_boost::mpl::bool_<true> &){
    ...
    template<class T>
    void save(const T & t){
        geofeatures_boost::io::ios_flags_saver fs(os);
        geofeatures_boost::io::ios_precision_saver ps(os);
