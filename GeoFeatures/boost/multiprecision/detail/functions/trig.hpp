...
void eval_sin(T& result, const T& x)
{
   ...
   else
   {
      ...
      for(geofeatures_boost::int32_t k = static_cast<geofeatures_boost::int32_t>(0); k < n_scale; k++)
      {
...
void hyp2F1(T& result, const T& a, const T& b, const T& c, const T& x)
{
  ...
  // There are no checks on input range or parameter boundaries.
   ...
   T lim;
   eval_ldexp(lim, result, 1 - geofeatures_boost::multiprecision::detail::digits2<number<T, et_on> >::value);
...
void eval_asin(T& result, const T& x)
{
   ...
   unsigned target_precision = geofeatures_boost::multiprecision::detail::digits2<number<T, et_on> >::value;
...
void eval_atan(T& result, const T& x)
{
   ...
   // Newton-Raphson iteration
   static const geofeatures_boost::int32_t double_digits10_minus_a_few = std::numeric_limits<double>::digits10 - 3;

   T s, c, t;
   for(geofeatures_boost::int32_t digits = double_digits10_minus_a_few; digits <= std::numeric_limits<number<T, et_on> >::digits10; digits *= 2)
   {
      eval_sin(s, result);
      eval_cos(c, result);
