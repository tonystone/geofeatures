...
namespace detail{
//
// Workaround for missing abs(geofeatures_boost::long_long_type) and abs(__int128) on some compilers:
//
template <class T>
BOOST_CONSTEXPR typename enable_if_c<(is_signed<T>::value || is_floating_point<T>::value), T>::type abs(T t) BOOST_NOEXCEPT
{
   ...
   // that val is the most negative value permitted in geofeatures_boost::long_long_type.
...
#define BOOST_MP_USING_ABS using geofeatures_boost::multiprecision::detail::abs;
...
template <class S>
void format_float_string(S& str, geofeatures_boost::intmax_t my_exp, geofeatures_boost::intmax_t digits, std::ios_base::fmtflags f, bool iszero)
{
   ...
   else if(!fixed || (my_exp >= 0))
   {
      ...
      //
      geofeatures_boost::intmax_t chars = str.size();
      chars = digits - chars;
      if(scientific)
         ++chars;
      if(chars > 0)
   ...
   {
      if(1 + my_exp > static_cast<geofeatures_boost::intmax_t>(str.size()))
      {
      ...
      else if(my_exp + 1 < static_cast<geofeatures_boost::intmax_t>(str.size()))
      {
         if(my_exp < 0)
         {
      ...
      if(fixed)
      {
         ...
         geofeatures_boost::intmax_t l = str.find('.') + 1;
         l = digits - (str.size() - l);
         if(l > 0)
   ...
   else
   {
      ...
      S e = geofeatures_boost::lexical_cast<S>(abs(my_exp));
...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost{ namespace math{ namespace tools{

template <class T>
struct promote_arg;

template <class tag, class A1, class A2, class A3, class A4>
struct promote_arg<geofeatures_boost::multiprecision::detail::expression<tag, A1, A2, A3, A4> >
{
   typedef typename geofeatures_boost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type type;
};

template <class R, class B, geofeatures_boost::multiprecision::expression_template_option ET>
inline R real_cast(const geofeatures_boost::multiprecision::number<B, ET>& val)
{
   return val.template convert_to<R>();
}

template <class R, class tag, class A1, class A2, class A3, class A4>
inline R real_cast(const geofeatures_boost::multiprecision::detail::expression<tag, A1, A2, A3, A4>& val)
{
   typedef typename geofeatures_boost::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type val_type;
...
namespace constants{
   ...
   template <class B, geofeatures_boost::multiprecision::expression_template_option ET>
   struct is_explicitly_convertible_from_string<geofeatures_boost::multiprecision::number<B, ET> >
   {
