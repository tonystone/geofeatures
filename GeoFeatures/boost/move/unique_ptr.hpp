...
struct deleter_types
{
   ...
   typedef ::geofeatures_boost::rv<D> &                                       deleter_arg_type2;
   #endif
};
...
struct unique_ptr_data
{
   ...
   unique_ptr_data(P p, BOOST_FWD_REF(U) d1) BOOST_NOEXCEPT
      : m_p(p), d(::geofeatures_boost::forward<U>(d1))
   {}
   ...
   : private D
{
   ...
   unique_ptr_data(P p, BOOST_FWD_REF(U) d) BOOST_NOEXCEPT
      : D(::geofeatures_boost::forward<U>(d)), m_p(p)
   {}
...
class is_rvalue_convertible
{
   ...
   #else
   typedef typename bmupmu::if_c
      < ::geofeatures_boost::has_move_emulation_enabled<T>::value && !bmupmu::is_reference<T>::value
...
class unique_ptr
{
      ...
      : m_data(p)
   {
      ...
      BOOST_STATIC_ASSERT(( !::geofeatures_boost::move_upmu::missing_virtual_destructor
   ...
   unique_ptr(Pointer p, BOOST_MOVE_SEEDOC(deleter_arg_type2) d2
      ...
      : m_data(p, ::geofeatures_boost::move(d2))
   {
      //If T is not an array type, element_type_t<Pointer> derives from T
      //it uses the default deleter and T has no virtual destructor, then you have a problem
      BOOST_STATIC_ASSERT(( !::geofeatures_boost::move_upmu::missing_virtual_destructor
   ...
   unique_ptr(BOOST_MOVE_DOC0PTR(bmupd::nullptr_type), BOOST_MOVE_SEEDOC(deleter_arg_type2) d2) BOOST_NOEXCEPT
      : m_data(pointer(), ::geofeatures_boost::move(d2))
   {}
   ...
   unique_ptr(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
      : m_data(u.release(), ::geofeatures_boost::move_if_not_lvalue_reference<D>(u.get_deleter()))
   {}
   ...
   unique_ptr( BOOST_RV_REF_BEG_IF_CXX11 unique_ptr<U, E> BOOST_RV_REF_END_IF_CXX11 u
      ...
      : m_data(u.release(), ::geofeatures_boost::move_if_not_lvalue_reference<E>(u.get_deleter()))
   {
   ...
   unique_ptr& operator=(BOOST_RV_REF(unique_ptr) u) BOOST_NOEXCEPT
   {
      ...
      m_data.deleter() = ::geofeatures_boost::move_if_not_lvalue_reference<D>(u.get_deleter());
      return *this;
   }
   ...
   {
      this->reset(u.release());
      m_data.deleter() = ::geofeatures_boost::move_if_not_lvalue_reference<E>(u.get_deleter());
      return *this;
   }
   ...
   {
      ::geofeatures_boost::adl_move_swap(m_data.m_p, u.m_data.m_p);
      ::geofeatures_boost::adl_move_swap(m_data.deleter(), u.m_data.deleter());
   }
};
