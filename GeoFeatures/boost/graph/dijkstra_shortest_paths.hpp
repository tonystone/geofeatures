...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost {
    ...
    struct dijkstra_bfs_visitor
    {
      ...
      void examine_edge(Edge e, Graph& g) {
        ...
        if (m_compare(m_combine(m_zero, get(m_weight, e)), m_zero)) 
            geofeatures_boost::throw_exception(negative_edge());
  ...
  namespace detail {
    ...
    struct vertex_property_map_generator_helper<Graph, IndexMap, Value, true> {
      typedef geofeatures_boost::iterator_property_map<Value*, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index, geofeatures_boost::scoped_array<Value>& array_holder) {
    ...
    struct vertex_property_map_generator_helper<Graph, IndexMap, Value, false> {
      typedef geofeatures_boost::vector_property_map<Value, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index, geofeatures_boost::scoped_array<Value>& array_holder) {
        return geofeatures_boost::make_vector_property_map<Value>(index);
      }
    };

    template <class Graph, class IndexMap, class Value>
    struct vertex_property_map_generator {
      typedef geofeatures_boost::is_base_and_derived<
                geofeatures_boost::vertex_list_graph_tag,
                typename geofeatures_boost::graph_traits<Graph>::traversal_category>
              known_num_vertices;
  ...
  namespace detail {
    ...
    struct default_color_map_generator_helper<Graph, IndexMap, true> {
      typedef geofeatures_boost::two_bit_color_map<IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        size_t nv = num_vertices(g);
        return geofeatures_boost::two_bit_color_map<IndexMap>(nv, index);
      }
    };

    template <class Graph, class IndexMap>
    struct default_color_map_generator_helper<Graph, IndexMap, false> {
      typedef geofeatures_boost::vector_property_map<geofeatures_boost::two_bit_color_type, IndexMap> type;
      static type build(const Graph& g, const IndexMap& index) {
        return geofeatures_boost::make_vector_property_map<geofeatures_boost::two_bit_color_type>(index);
      }
    };
     ...
     DijkstraVisitor vis, ColorMap color)
  {
    ...
    MutableQueue Q(num_vertices(g), icmp, index_map);
      ...
      geofeatures_boost::scoped_array<std::size_t> index_in_heap_map_holder;
      typedef
  ...
  {
    geofeatures_boost::two_bit_color_map<IndexMap> color(num_vertices(g), index_map);
     ...
     DijkstraVisitor vis, ColorMap color)
  {
    ...
    for (geofeatures_boost::tie(ui, ui_end) = vertices(g); ui != ui_end; ++ui) {
