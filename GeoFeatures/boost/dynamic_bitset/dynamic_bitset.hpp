...
public:
    ...
    :m_bits(alloc),
     m_num_bits(0)
    {
        using geofeatures_boost::detail::dynamic_bitset_impl::value_to_type;
        using geofeatures_boost::detail::dynamic_bitset_impl::is_numeric;
    ...
    void m_append(BlockInputIterator first, BlockInputIterator last, std::forward_iterator_tag)
    {
        ...
        std::size_t d = geofeatures_boost::detail::distance(first, last);
...
private:
                                 ...
                                 const Allocator& alloc*/)
    {
        ...
        typedef geofeatures_boost::detail::dynamic_bitset_impl
...
dynamic_bitset(dynamic_bitset<Block, Allocator>&& b)
  : m_bits(geofeatures_boost::move(b.m_bits)), m_num_bits(geofeatures_boost::move(b.m_num_bits))
{
...
{
    if (geofeatures_boost::addressof(b) == this) { return *this; }

    m_bits = geofeatures_boost::move(b.m_bits);
    m_num_bits = geofeatures_boost::move(b.m_num_bits);
    // Required so that assert(m_check_invariants()); works.
    assert((b.m_bits = buffer_type()).empty());
    b.m_num_bits = 0;
    return *this;
...
to_ulong() const
{
  ...
  if (find_next(ulong_width - 1) != npos)
    BOOST_THROW_EXCEPTION(std::overflow_error("geofeatures_boost::dynamic_bitset::to_ulong overflow"));
...
dynamic_bitset<Block, Allocator>::m_do_find_from(size_type first_block) const
{
    ...
    return i * bits_per_block + static_cast<size_type>(geofeatures_boost::lowest_bit(m_bits[i]));
