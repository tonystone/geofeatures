...
///
/// All symbols in the Execution Monitor implementation are located in the namespace geofeatures_boost. To use the Execution Monitor you need to:
...
typedef geofeatures_boost::shared_ptr<translator_holder_base> translator_holder_base_ptr;

class BOOST_TEST_DECL translator_holder_base {
protected:
    typedef geofeatures_boost::unit_test::const_string const_string;
public:
    // Constructor
    ...
    virtual int operator()( geofeatures_boost::function<int ()> const& F ) = 0;
    ...
    translator_holder_base_ptr erase( translator_holder_base_ptr this_, geofeatures_boost::type<ExceptionType>* = 0 )
    {
        if( m_next )
...
public:
    ...
    /// @see vexecute
    int         execute( geofeatures_boost::function<int ()> const& F );
    ...
    /// @param[in] F  Function to monitor
    /// @see execute
    void         vexecute( geofeatures_boost::function<void ()> const& F );
    // @}
    ...
    void        register_exception_translator( ExceptionTranslator const& tr, const_string tag = const_string(), geofeatures_boost::type<ExceptionType>* = 0 );
    ...
    template<typename ExceptionType>
    void        erase_exception_translator( geofeatures_boost::type<ExceptionType>* = 0 )
    {
...
private:
    ...
    int         catch_signals( geofeatures_boost::function<int ()> const& F );

    // Data members
    detail::translator_holder_base_ptr  m_custom_translators;
    geofeatures_boost::scoped_array<char>           m_alt_stack;
}; // execution_monitor
...
public:
    ...
    // translator holder interface
    virtual int operator()( geofeatures_boost::function<int ()> const& F )
    {
        BOOST_TEST_IMPL_TRY {
        ...
        BOOST_TEST_IMPL_CATCH( ExceptionType, e ) {
            m_translator( e );
            return geofeatures_boost::exit_exception_failure;
        }
    }
#ifndef BOOST_NO_RTTI
...
template<typename ExceptionType, typename ExceptionTranslator>
void
execution_monitor::register_exception_translator( ExceptionTranslator const& tr, const_string tag, geofeatures_boost::type<ExceptionType>* )
{
...
#define BOOST_TEST_SYS_ASSERT( exp ) \
    if( (exp) ) ; \
    else BOOST_TEST_IMPL_THROW( ::geofeatures_boost::system_error( BOOST_STRINGIZE( exp ) ) )
