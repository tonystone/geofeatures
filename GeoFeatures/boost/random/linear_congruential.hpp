...
public:
    ...
    void discard(geofeatures_boost::uintmax_t z)
    {
        ...
        if(b_gcd == 1) {
            ...
            geofeatures_boost::uintmax_t exponent = z;
            while(exponent != 0) {
                if(exponent % 2 == 1) {
                    a_zm1_over_gcd =
                        mod_type::mult_add(
...
{
public:
    typedef geofeatures_boost::uint32_t result_type;
    ...
    /** Advances the state of the generator by @c z. */
    void discard(geofeatures_boost::uintmax_t z) { lcf.discard(z); }
...
private:
    ...
    static geofeatures_boost::uint64_t cnv(geofeatures_boost::uint32_t x)
