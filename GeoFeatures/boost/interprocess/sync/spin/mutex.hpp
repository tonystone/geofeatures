...
class spin_mutex
{
   ...
   void take_ownership(){};
   private:
   volatile geofeatures_boost::uint32_t m_s;
};
...
{
   geofeatures_boost::uint32_t prev_s = ipcdetail::atomic_cas32(const_cast<geofeatures_boost::uint32_t*>(&m_s), 1, 0);
   return m_s == 1 && prev_s == 0;
}

inline bool spin_mutex::timed_lock(const geofeatures_boost::posix_time::ptime &abs_time)
{  return ipcdetail::try_based_timed_lock(*this, abs_time); }

inline void spin_mutex::unlock(void)
{  ipcdetail::atomic_cas32(const_cast<geofeatures_boost::uint32_t*>(&m_s), 0, 1);   }
