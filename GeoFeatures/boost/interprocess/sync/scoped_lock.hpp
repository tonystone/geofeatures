...
class scoped_lock
{
   ...
   scoped_lock(mutex_type& m, const geofeatures_boost::posix_time::ptime& abs_time)
   ...
   //!   can be moved with the expression: "geofeatures_boost::move(lock);". This
   ...
   //!   the expression: "geofeatures_boost::move(lock);" This constructor may block if
   ...
   template<class T>
   scoped_lock(BOOST_RV_REF(upgradable_lock<T>) upgr, geofeatures_boost::posix_time::ptime &abs_time
               , typename ipcdetail::enable_if< ipcdetail::is_same<T, Mutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
      upgradable_lock<mutex_type> &u_lock = upgr;
   ...
   bool timed_lock(const geofeatures_boost::posix_time::ptime& abs_time)
   {
      if(!mp_mutex || m_locked)
         throw lock_exception();
