   ...
   : public managed_open_or_create_impl_device_holder<StoreDevice, DeviceAbstraction>
{
   ...
   static const std::size_t
      ManagedOpenOrCreateUserOffset =
         ct_rounded_size
            < sizeof(geofeatures_boost::uint32_t)
            , MemAlignment ? (MemAlignment) :
               (::geofeatures_boost::container::container_detail::alignment_of
   ...
   {
      managed_open_or_create_impl tmp(geofeatures_boost::move(moved));
      this->swap(tmp);
      return *this;
   }
       ...
       ConstructFunc construct_func)
   {
      ...
      if(created){
         try{
            ...
            geofeatures_boost::uint32_t *patomic_word = 0;  //avoid gcc warning
            patomic_word = static_cast<geofeatures_boost::uint32_t*>(region.get_address());
            geofeatures_boost::uint32_t previous = atomic_cas32(patomic_word, InitializingSegment, UninitializedSegment);
         ...
         geofeatures_boost::uint32_t *patomic_word = static_cast<geofeatures_boost::uint32_t*>(region.get_address());
         geofeatures_boost::uint32_t value = atomic_read32(patomic_word);
      ...
      if(StoreDevice){
         this->DevHolder::get_device() = geofeatures_boost::move(dev);
      }
   }
