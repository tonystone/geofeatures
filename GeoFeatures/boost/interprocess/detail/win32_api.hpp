...
struct decimal
{
    ...
    union {
        ...
        ::geofeatures_boost::ulong_long_type Lo64;
    };
};
...
union system_timeofday_information
{
   struct data_t
   {
      ...
      ::geofeatures_boost::ulong_long_type ullBootTimeBias;
      ::geofeatures_boost::ulong_long_type ullSleepTimeBias;
   } data;
...
inline void * create_file_mapping (void * handle, unsigned long access, ::geofeatures_boost::ulong_long_type file_offset, const char * name, interprocess_security_attributes *psec)
{
   const unsigned long high_size(file_offset >> 32), low_size((geofeatures_boost::uint32_t)file_offset);
...
inline void *map_view_of_file_ex(void *handle, unsigned long file_access, ::geofeatures_boost::ulong_long_type offset, std::size_t numbytes, void *base_addr)
{
   const unsigned long offset_low  = (unsigned long)(offset & ((::geofeatures_boost::ulong_long_type)0xFFFFFFFF));
...
class nt_query_mem_deleter
{
   ...
   static const std::size_t rename_suffix =
      (SystemTimeOfDayInfoLength + sizeof(unsigned long) + sizeof(geofeatures_boost::uint32_t))*2;
...
inline bool unlink_file(const char *filename)
{
   ...
   try{
      ...
      handle_closer h_closer(fh);
      {
         ...
         static volatile geofeatures_boost::uint32_t u32_count = 0;
         interlocked_decrement(reinterpret_cast<volatile long*>(&u32_count));
         buffer_to_wide_str(const_cast<const geofeatures_boost::uint32_t *>(&u32_count), sizeof(geofeatures_boost::uint32_t), &pfri->FileName[filename_string_length]);
         filename_string_length += sizeof(geofeatures_boost::uint32_t)*2;
