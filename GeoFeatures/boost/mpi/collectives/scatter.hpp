...
{
  if (comm.rank() == root)
    ::geofeatures_boost::mpi::scatter<T>(comm, &in_values[0], out_value, root);
  else
    ::geofeatures_boost::mpi::scatter<T>(comm, static_cast<const T*>(0), out_value, 
                             root);
}
...
{
  if (comm.rank() == root)
    ::geofeatures_boost::mpi::scatter(comm, &in_values[0], out_values, n, root);
  else
    ::geofeatures_boost::mpi::scatter(comm, static_cast<const T*>(0), out_values, 
                                    n, root);
}
