 ...
 * geofeatures_boost::throw_exception to throw an exception or abort, depending on
 * BOOST_NO_EXCEPTIONS.
 */
#define BOOST_MPI_CHECK_RESULT( MPIFunc, Args )                         \
 {                                                                      \
   ...
   if (_check_result != MPI_SUCCESS)                                    \
     geofeatures_boost::throw_exception(geofeatures_boost::mpi::exception(#MPIFunc,   \
                                                             _check_result)); \
 }
