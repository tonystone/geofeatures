...
class BOOST_MPI_DECL mpi_datatype_map
 : public geofeatures_boost::noncopyable
{
...
public:
  ...
  template <class T>
  MPI_Datatype datatype(const T& x = T(), typename geofeatures_boost::enable_if<is_mpi_builtin_datatype<T> >::type* =0)
  {
    return get_mpi_datatype<T>(x);
  }

  template <class T>
  MPI_Datatype datatype(const T& x =T(), typename geofeatures_boost::disable_if<is_mpi_builtin_datatype<T> >::type* =0 )
  {
