...
public:
    ...
    virtual void load(geofeatures_boost::long_long_type & t) = 0;
    virtual void load(geofeatures_boost::ulong_long_type & t) = 0;
    ...
    template<class T>
    void load_override(
        const geofeatures_boost::serialization::nvp< T > & t
    ){
        load_start(t.name());
...
public:
    ...
    virtual const detail::basic_pointer_iserializer * load_pointer(
        ...
        const detail::basic_pointer_iserializer * (*finder)(
            const geofeatures_boost::serialization::extended_type_info & type
        )
    ) = 0;
};
...
// required by export
BOOST_SERIALIZATION_REGISTER_ARCHIVE(geofeatures_boost::archive::polymorphic_iarchive)
