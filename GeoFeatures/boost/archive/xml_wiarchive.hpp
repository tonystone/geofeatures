...
#else
    #include <codecvt>
    namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost { namespace archive { namespace detail {
...
protected:
    ...
    #endif
#endif
    geofeatures_boost::scoped_ptr<xml_wgrammar> gimpl;
    std::wistream & get_is(){
        return is;
    }
    template<class T>
    ...
    void 
    load(geofeatures_boost::serialization::item_version_type & t){
        unsigned int v;
        load(v);
        t = geofeatures_boost::serialization::item_version_type(v);
    }
...
// required by export
BOOST_SERIALIZATION_REGISTER_ARCHIVE(geofeatures_boost::archive::xml_wiarchive)
