...
operator<<(basic_ostream<CharType> & os, geofeatures_boost::uint64_t t){
...
template<class CharType>
basic_ostream<CharType> & 
operator<<(basic_ostream<CharType> &os, geofeatures_boost::int64_t t){
    if(0 <= t){
        os << static_cast<geofeatures_boost::uint64_t>(t);
    }
    else{
        os.put('-');
        os << -t;
...
operator>>(basic_istream<CharType> &is, geofeatures_boost::int64_t & t){
    CharType d;
    do{
        d = is.get();
    }
...
template<class CharType>
basic_istream<CharType> & 
operator>>(basic_istream<CharType> &is, geofeatures_boost::uint64_t & t){
    geofeatures_boost::int64_t it;
    is >> it;
    t = it;
    return is;
}
