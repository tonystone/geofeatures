...
struct varray_traits
{
    ...
    typedef geofeatures_boost::false_type use_memop_in_swap_and_move;
    typedef geofeatures_boost::false_type use_optimized_swap;
    typedef geofeatures_boost::false_type disable_trivial_init;
};
...
struct checker
{
    ...
    static inline void check_capacity(Varray const& v, size_type s)
    {
        ...
        ::geofeatures_boost::ignore_unused_variable_warning(v);
        ::geofeatures_boost::ignore_unused_variable_warning(s);
    }
    ...
    static inline void throw_out_of_bounds(Varray const& v, size_type i)
    {
        ...
        ::geofeatures_boost::ignore_unused_variable_warning(v);
        ::geofeatures_boost::ignore_unused_variable_warning(i);
    }
    ...
    static inline void check_iterator_end_neq(Varray const& v, const_iterator position)
    {
        ...
        ::geofeatures_boost::ignore_unused_variable_warning(v);
        ::geofeatures_boost::ignore_unused_variable_warning(position);
    }
...
varray is a sequence container like geofeatures_boost::container::vector with contiguous storage that can
change in size, along with the static allocation, low overhead, and fixed capacity of geofeatures_boost::array.
...
class varray
{
    ...
    BOOST_MPL_ASSERT_MSG(
        ( geofeatures_boost::is_unsigned<typename vt::size_type>::value &&
          sizeof(typename geofeatures_boost::uint_value_t<Capacity>::least) <= sizeof(typename vt::size_type) ),
        SIZE_TYPE_IS_TOO_SMALL_FOR_SPECIFIED_CAPACITY,
        (varray)
    );

    typedef geofeatures_boost::aligned_storage<
        sizeof(Value[Capacity]),
        geofeatures_boost::alignment_of<Value[Capacity]>::value
...
public:
    ...
    varray & operator=(varray<Value, C> & sv)
    {
        ...
        this->operator=(static_cast<const ::geofeatures_boost::rv<other> &>(const_cast<const other &>(sv)));
        return *this;
    }
#endif
...
public:
    ...
    typedef geofeatures_boost::reverse_iterator<iterator> reverse_iterator;
    //! @brief The const reverse iterator.
    typedef geofeatures_boost::reverse_iterator<const_iterator> const_reverse_iterator;
    ...
    varray(Iterator first, Iterator last)
        : m_size(0)
    {
        BOOST_CONCEPT_ASSERT((geofeatures_boost_concepts::ForwardTraversal<Iterator>)); // Make sure you passed a ForwardIterator
    ...
    //!
    //! @par Throws
    //!   @li If \c geofeatures_boost::has_nothrow_move<Value>::value is \c true and Value's move constructor throws.
    //!   @li If \c geofeatures_boost::has_nothrow_move<Value>::value is \c false and Value's copy constructor throws.
    //! @internal
    ...
    //!
    //! @par Throws
    //!   @li If \c geofeatures_boost::has_nothrow_move<Value>::value is \c true and Value's move constructor or move assignment throws.
    //!   @li If \c geofeatures_boost::has_nothrow_move<Value>::value is \c false and Value's copy constructor or copy assignment throws.
    //! @internal
    //!   @li It throws only if \c use_memop_in_swap_and_move is \c false_type - default.
    //! @endinternal
    //!
    ...
    void push_back(BOOST_RV_REF(value_type) value)
    {
        ...
        namespace sv = varray_detail;
        sv::construct(dti(), this->end(), ::geofeatures_boost::move(value));                           // may throw
        ++m_size; // update end
    }
    ...
    iterator insert(iterator position, value_type const& value)
    {
        ...
        else
        {
            ...
            sv::construct(dti(), this->end(), geofeatures_boost::move(r));                      // may throw
            ++m_size; // update end
    ...
    iterator insert(iterator position, BOOST_RV_REF(value_type) value)
    {
        ...
        if ( position == this->end() )
        {
            sv::construct(dti(), position, geofeatures_boost::move(value));                     // may throw
            ++m_size; // update end
        }
        else
        {
            ...
            ++m_size; // update end
            sv::move_backward(position, this->end() - 2, this->end() - 1);          // may throw
            sv::assign(position, geofeatures_boost::move(value));                               // may throw
        }
    ...
    iterator insert(iterator position, Iterator first, Iterator last)
    {
        ...
        typedef typename geofeatures_boost::iterator_traversal<Iterator>::type traversal;
    ...
    void emplace_back(BOOST_FWD_REF(Args) ...args)
    {
        ...
        namespace sv = varray_detail;
        sv::construct(dti(), this->end(), ::geofeatures_boost::forward<Args>(args)...);                // may throw
        ++m_size; // update end
    }
    ...
    iterator emplace(iterator position, BOOST_FWD_REF(Args) ...args)
    {
        ...
        if ( position == this->end() )
        {
            sv::construct(dti(), position, ::geofeatures_boost::forward<Args>(args)...);               // may throw
            ++m_size; // update end
        }
        else
        {
            ...
            sv::construct(dti(), val_p, ::geofeatures_boost::forward<Args>(args)...);                  // may throw
            sv::scoped_destructor<value_type> d(val_p);
            sv::assign(position, ::geofeatures_boost::move(*val_p));                            // may throw
        }
    ...
    Value * data()
    {
        return geofeatures_boost::addressof(*(this->ptr()));
    }
...
private:
    ...
    //   Linear O(N).
    template <std::size_t C>
    void move_ctor_dispatch(varray<value_type, C> & other, geofeatures_boost::true_type /*use_memop*/)
    {
    ...
    // @par Throws
    //   @li If geofeatures_boost::has_nothrow_move<Value>::value is true and Value's move constructor throws
    //   @li If geofeatures_boost::has_nothrow_move<Value>::value is false and Value's copy constructor throws.
    // @par Complexity
    //   Linear O(N).
    template <std::size_t C>
    void move_ctor_dispatch(varray<value_type, C> & other, geofeatures_boost::false_type /*use_memop*/)
    {
        namespace sv = varray_detail;
    ...
    //   Linear O(N).
    template <std::size_t C>
    void move_assign_dispatch(varray<value_type, C> & other, geofeatures_boost::true_type /*use_memop*/)
    {
        this->clear();
    ...
    // @par Throws
    //   @li If geofeatures_boost::has_nothrow_move<Value>::value is true and Value's move constructor or move assignment throws
    //   @li If geofeatures_boost::has_nothrow_move<Value>::value is false and Value's copy constructor or move assignment throws.
    // @par Complexity
    //   Linear O(N).
    template <std::size_t C>
    void move_assign_dispatch(varray<value_type, C> & other, geofeatures_boost::false_type /*use_memop*/)
    {
        namespace sv = varray_detail;
    ...
    //   Linear O(N).
    template <std::size_t C>
    void swap_dispatch(varray<value_type, C> & other, geofeatures_boost::true_type const& /*use_optimized_swap*/)
    {
        typedef typename
    ...
    //   Linear O(N).
    template <std::size_t C>
    void swap_dispatch(varray<value_type, C> & other, geofeatures_boost::false_type const& /*use_optimized_swap*/)
    {
        namespace sv = varray_detail;
    ...
    // @par Complexity
    //   Linear O(N).
    void swap_dispatch_impl(iterator first_sm, iterator last_sm, iterator first_la, iterator last_la, geofeatures_boost::true_type const& /*use_memop*/)
    {
        ...
        {
            geofeatures_boost::aligned_storage<
                sizeof(value_type),
                geofeatures_boost::alignment_of<value_type>::value
            > temp_storage;
            value_type * temp_ptr = reinterpret_cast<value_type*>(temp_storage.address());

            ::memcpy(temp_ptr, geofeatures_boost::addressof(*first_sm), sizeof(value_type));
            ::memcpy(geofeatures_boost::addressof(*first_sm), geofeatures_boost::addressof(*first_la), sizeof(value_type));
            ::memcpy(geofeatures_boost::addressof(*first_la), temp_ptr, sizeof(value_type));
        }
    ...
    // @par Complexity
    //   Linear O(N).
    void swap_dispatch_impl(iterator first_sm, iterator last_sm, iterator first_la, iterator last_la, geofeatures_boost::false_type const& /*use_memop*/)
    {
        //BOOST_GEOMETRY_INDEX_ASSERT(std::distance(first_sm, last_sm) <= std::distance(first_la, last_la),
        //                            "incompatible ranges");
        ...
        for (; first_sm != last_sm ; ++first_sm, ++first_la)
        {
            //geofeatures_boost::swap(*first_sm, *first_la);                                    // may throw
            value_type temp(geofeatures_boost::move(*first_sm));                                // may throw
            *first_sm = geofeatures_boost::move(*first_la);                                     // may throw
            *first_la = geofeatures_boost::move(temp);                                          // may throw
        }
    ...
    //   Linear O(N).
    template <typename Iterator>
    void insert_dispatch(iterator position, Iterator first, Iterator last, geofeatures_boost::random_access_traversal_tag const&)
    {
        BOOST_CONCEPT_ASSERT((geofeatures_boost_concepts::RandomAccessTraversal<Iterator>)); // Make sure you passed a RandomAccessIterator
        
        errh::check_iterator_end_eq(*this, position);
        
        typename geofeatures_boost::iterator_difference<Iterator>::type
    ...
    //   Linear O(N).
    template <typename Iterator>
    void assign_dispatch(Iterator first, Iterator last, geofeatures_boost::random_access_traversal_tag const& /*not_random_access*/)
    {
        namespace sv = varray_detail;
...
public:
    ...
    // nothrow
    Value * data() { return geofeatures_boost::addressof(*(this->ptr())); }
    const Value * data() const { return geofeatures_boost::addressof(*(this->ptr())); }
