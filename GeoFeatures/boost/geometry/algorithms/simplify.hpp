...
struct simplify_range_insert
{
                             ...
                             Distance const& max_distance, Strategy const& strategy)
    {
        ...
        if (geofeatures_boost::size(range) <= 2 || max_distance < 0)
        {
            std::copy(geofeatures_boost::begin(range), geofeatures_boost::end(range), out);
        }
        else
        {
...
struct simplify_copy
{
                             ...
                             Distance const& , Strategy const& )
    {
        std::copy
            (
                geofeatures_boost::begin(range), geofeatures_boost::end(range), std::back_inserter(out)
            );
    }
};
...
struct simplify_range
{
                    ...
                    Distance const& max_distance, Strategy const& strategy)
    {
        ...
        */
        ...
        if (geofeatures_boost::size(range) <= int(Minimum) || max_distance < 0.0)
        {
...
private:
                    ...
                    Distance const& max_distance, Strategy const& strategy)
    {
        traits::resize<InteriorRingsOut>::apply(interior_rings_out,
            geofeatures_boost::size(interior_rings_in));

        iterate<Minimum>(
            geofeatures_boost::begin(interior_rings_in), geofeatures_boost::end(interior_rings_in),
            geofeatures_boost::begin(interior_rings_out),
...
struct simplify_multi
{
    ...
    static inline void apply(MultiGeometry const& multi, MultiGeometry& out,
                    Distance const& max_distance, Strategy const& strategy)
    {
        traits::resize<MultiGeometry>::apply(out, geofeatures_boost::size(multi));

        typename geofeatures_boost::range_iterator<MultiGeometry>::type it_out
                = geofeatures_boost::begin(out);
        for (typename geofeatures_boost::range_iterator<MultiGeometry const>::type
                it_in = geofeatures_boost::begin(multi);
             it_in != geofeatures_boost::end(multi);
             ++it_in, ++it_out)
        {
...
struct simplify<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    template <typename Distance, typename Strategy>
    struct visitor: geofeatures_boost::static_visitor<void>
    {
    ...
    apply(geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const& geometry,
          geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>& out,
          Distance const& max_distance,
          Strategy const& strategy)
    {
        geofeatures_boost::apply_visitor(
