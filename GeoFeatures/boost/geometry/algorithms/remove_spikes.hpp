...
struct range_remove_spikes
{
    ...
    {
        std::size_t n = geofeatures_boost::size(range);
        ...
        for (typename geofeatures_boost::range_iterator<Range const>::type it = geofeatures_boost::begin(range);
            it != geofeatures_boost::end(range); ++it)
        {
            // Add point
...
struct multi_remove_spikes
{
    ...
    {
        for (typename geofeatures_boost::range_iterator<MultiGeometry>::type
...
struct remove_spikes<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    ...
    static inline void apply(geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>& geometry)
    {
        geofeatures_boost::apply_visitor(visitor(), geometry);
    }
};
