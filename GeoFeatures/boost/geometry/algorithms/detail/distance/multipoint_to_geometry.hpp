...
struct multipoint_to_multipoint
{
    ...
    {
        if (geofeatures_boost::size(multipoint2) < geofeatures_boost::size(multipoint1))

        {
            return point_or_segment_range_to_geometry_rtree
                <
                    typename geofeatures_boost::range_iterator<MultiPoint2 const>::type,
                    MultiPoint1,
                    Strategy
                >::apply(geofeatures_boost::begin(multipoint2),
                         geofeatures_boost::end(multipoint2),
                         multipoint1,
                         strategy);
        }
        ...
        return point_or_segment_range_to_geometry_rtree
            <
                typename geofeatures_boost::range_iterator<MultiPoint1 const>::type,
                MultiPoint2,
                Strategy
            >::apply(geofeatures_boost::begin(multipoint1),
                     geofeatures_boost::end(multipoint1),
                     multipoint2,
                     strategy);
    }
};
...
struct multipoint_to_linear
{
                                    ...
                                    Strategy const& strategy)
    {
        return detail::distance::point_or_segment_range_to_geometry_rtree
            <
                typename geofeatures_boost::range_iterator<MultiPoint const>::type,
                Linear,
                Strategy
            >::apply(geofeatures_boost::begin(multipoint),
                     geofeatures_boost::end(multipoint),
                     linear,
                     strategy);
    }
