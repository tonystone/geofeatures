        ...
        Turns& turns)
{
    ...
    for (typename geofeatures_boost::range_iterator<AngleCollection const>::type it =
...
inline std::size_t assign_cluster_indices(AngleCollection& sorted, Point const& origin)
{
    ...
    BOOST_GEOMETRY_ASSERT(geofeatures_boost::size(sorted) >= 4u);

    angle_equal_to<Point> comparator(origin);
    typename geofeatures_boost::range_iterator<AngleCollection>::type it = sorted.begin();

    std::size_t cluster_index = 0;
    it->cluster_index = cluster_index;
    typename geofeatures_boost::range_iterator<AngleCollection>::type previous = it++;
...
{
    BOOST_GEOMETRY_ASSERT(geofeatures_boost::size(sorted) >= 4u && cluster_size > 0);
    ...
    for (typename geofeatures_boost::range_iterator<AngleCollection const>::type it = sorted.begin();
        it != sorted.end(); ++it)
    {
        if (it->incoming)
        {
    ...
    for (typename geofeatures_boost::range_iterator<AngleCollection>::type it = sorted.begin();
        it != sorted.end(); ++it)
    {
