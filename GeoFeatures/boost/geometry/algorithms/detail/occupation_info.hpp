...
inline void move_index(Pieces const& pieces, signed_size_type& index, signed_size_type& piece_index, int direction)
{
    ...
    BOOST_GEOMETRY_ASSERT(
        piece_index >= 0
        && piece_index < static_cast<signed_size_type>(geofeatures_boost::size(pieces)) );
    BOOST_GEOMETRY_ASSERT(
        index >= 0
        && index < static_cast<signed_size_type>(geofeatures_boost::size(pieces[piece_index].robust_ring)));
    ...
    if (direction == -1 && index < 0)
    {
        ...
        if (piece_index < 0)
        {
            piece_index = geofeatures_boost::size(pieces) - 1;
        }
        index = geofeatures_boost::size(pieces[piece_index].robust_ring) - 1;
    }
    if (direction == 1
        && index >= static_cast<signed_size_type>(geofeatures_boost::size(pieces[piece_index].robust_ring)))
    {
        piece_index++;
        if (piece_index >= static_cast<signed_size_type>(geofeatures_boost::size(pieces)))
        {
            piece_index = 0;
        }
        index = 0;
