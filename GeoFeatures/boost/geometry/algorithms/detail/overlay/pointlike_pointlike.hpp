...
struct copy_points<PointOut, MultiPointIn, multi_point_tag>
{
    ...
    static inline void apply(MultiPointIn const& multi_point_in,
                             OutputIterator& oit)
    {
        for (typename geofeatures_boost::range_iterator<MultiPointIn const>::type
                 it = geofeatures_boost::begin(multi_point_in);
             it != geofeatures_boost::end(multi_point_in); ++it)
        {
            PointOut point_out;
...
struct multipoint_point_point
{
                                       ...
                                       Strategy const&)
    {
        ...
        for (typename geofeatures_boost::range_iterator<MultiPoint const>::type
                 it = geofeatures_boost::begin(multipoint);
             it != geofeatures_boost::end(multipoint); ++it)
        {
            action_selector_pl_pl
                <
                    PointOut, OverlayType
...
struct multipoint_multipoint_point
{
                                       ...
                                       Strategy const& strategy)
    {
        if ( OverlayType != overlay_difference
             && geofeatures_boost::size(multipoint1) > geofeatures_boost::size(multipoint2) )
        {
        ...
        std::vector<typename geofeatures_boost::range_value<MultiPoint2>::type>
            points2(geofeatures_boost::begin(multipoint2), geofeatures_boost::end(multipoint2));

        std::sort(points2.begin(), points2.end(), detail::relate::less());

        for (typename geofeatures_boost::range_iterator<MultiPoint1 const>::type
                 it1 = geofeatures_boost::begin(multipoint1);
             it1 != geofeatures_boost::end(multipoint1); ++it1)
        {
