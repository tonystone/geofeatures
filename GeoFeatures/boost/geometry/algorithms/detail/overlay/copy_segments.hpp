...
struct copy_segments_ring
{
            ...
            RangeOut& current_output)
    {
        ...
        typedef typename geofeatures_boost::range_iterator<rview_type const>::type iterator;
        ...
        // Sanity check
        BOOST_GEOMETRY_ASSERT(from_index < static_cast<signed_size_type>(geofeatures_boost::size(view)));

        ec_iterator it(geofeatures_boost::begin(view), geofeatures_boost::end(view),
                    geofeatures_boost::begin(view) + from_index);
        ...
        signed_size_type const count = from_index <= to_index
            ...
            : static_cast<signed_size_type>(geofeatures_boost::size(view))
...
private:
    ...
    static inline void append_to_output(RangeOut& current_output,
                                        Point const& point,
                                        RobustPolicy const& robust_policy,
                                        geofeatures_boost::true_type const&)
    {
    ...
    static inline void append_to_output(RangeOut& current_output,
                                        Point const& point,
                                        RobustPolicy const&,
                                        geofeatures_boost::false_type const&)
    {
...
public:
            ...
            RangeOut& current_output)
    {
        ...
        if ( from_index > to_index
          || from_index < 0
          || to_index >= static_cast<signed_size_type>(geofeatures_boost::size(ls)) )
        {
            return;
        }
        ...
        typename geofeatures_boost::range_iterator<LineString const>::type
            it = geofeatures_boost::begin(ls) + from_index;

        for (signed_size_type i = 0; i < count; ++i, ++it)
        {
            append_to_output(current_output, *it, robust_policy,
                             geofeatures_boost::integral_constant<bool, RemoveSpikes>());
        }
    }
};
...
struct copy_segments_box
{
            ...
            RangeOut& current_output)
    {
        ...
        geofeatures_boost::array<typename point_type<Box>::type, 5> bp;
            ...
            RangeOut& current_output)
    {
        ...
        BOOST_GEOMETRY_ASSERT
            (
                ...
                && static_cast<std::size_t>(seg_id.multi_index) < geofeatures_boost::size(multi_geometry)
            );
