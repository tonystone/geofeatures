...
public:
    ...
    {
        if (!geofeatures_boost::empty(line_out))
        {
            *out = line_out;
            ++out;
...
{
    if (geofeatures_boost::begin(range) == geofeatures_boost::end(range))
    {
        return out;
    }
    ...
    iterator_type vertex = geofeatures_boost::begin(range);
    for(iterator_type previous = vertex++;
            vertex != geofeatures_boost::end(range);
            ++previous, ++vertex)
    {
        point_type p1, p2;
