...
struct buffered_piece_collection
{
    ...
    struct robust_original
    {
            ...
            , m_has_interiors(has_interiors)
        {
            ...
            typedef geofeatures_boost::mpl::vector_c<std::size_t, 1> dimensions;
    ...
    )
    {
        ...
        typedef typename geofeatures_boost::range_iterator<turn_vector_type>::type
            iterator_type;

        for (iterator_type it = geofeatures_boost::begin(m_turns);
            it != geofeatures_boost::end(m_turns);
            ++it)
        {
    ...
    {
        for (typename geofeatures_boost::range_iterator<turn_vector_type>::type it =
            geofeatures_boost::begin(m_turns); it != geofeatures_boost::end(m_turns); ++it)
        {
    ...
    inline bool assert_indices_in_robust_rings() const
    {
        ...
        for (typename geofeatures_boost::range_iterator<turn_vector_type const>::type it =
            geofeatures_boost::begin(m_turns); it != geofeatures_boost::end(m_turns); ++it)
        {
            for (int i = 0; i < 2; i++)
            {
    ...
    inline void insert_rescaled_piece_turns()
    {
        ...
        std::size_t index = 0;
        for (typename geofeatures_boost::range_iterator<turn_vector_type>::type it =
            geofeatures_boost::begin(m_turns); it != geofeatures_boost::end(m_turns); ++it, ++index)
        {
        ...
        for (typename geofeatures_boost::range_iterator<piece_vector_type>::type
                it = geofeatures_boost::begin(m_pieces); it != geofeatures_boost::end(m_pieces); ++it)
        {
            piece& pc = *it;
            ...
            if (! pc.robust_turns.empty())
            {
                ...
                for (typename geofeatures_boost::range_reverse_iterator<const std::vector<robust_turn> >::type
                        rit = geofeatures_boost::const_rbegin(pc.robust_turns);
                    rit != geofeatures_boost::const_rend(pc.robust_turns);
                    ++rit, --index_offset)
                {
                    ...
                    pc.robust_ring.insert(geofeatures_boost::begin(pc.robust_ring) + index_in_vector, rit->point);
                    pc.offsetted_count++;
    ...
    {
        for (typename piece_vector_type::iterator it = geofeatures_boost::begin(m_pieces);
            it != geofeatures_boost::end(m_pieces);
            ++it)
        {
    ...
    {
        for(typename geofeatures_boost::range_iterator<sections_type>::type it
                = geofeatures_boost::begin(monotonic_sections);
            it != geofeatures_boost::end(monotonic_sections);
            ++it)
        {
    ...
    inline void start_new_ring()
    {
        ...
        m_first_piece_index = static_cast<signed_size_type>(geofeatures_boost::size(m_pieces));
    }
    ...
    inline void update_closing_point()
    {
        ...
        if (! geofeatures_boost::empty(added))
        {
            ...
            buffered_ring<Ring>& ring)
    {
        ...
        BOOST_GEOMETRY_ASSERT(geofeatures_boost::size(m_pieces) > 0);
        if (! ring.empty()
    ...
    inline void finish_ring(bool is_interior = false, bool has_interiors = false)
    {
        ...
        if (m_first_piece_index < static_cast<signed_size_type>(geofeatures_boost::size(m_pieces)))
        {
            // If piece was added
            // Reassign left-of-first and right-of-last
            geometry::range::at(m_pieces, m_first_piece_index).left_index
                    = static_cast<signed_size_type>(geofeatures_boost::size(m_pieces)) - 1;
    ...
    {
        BOOST_GEOMETRY_ASSERT(geofeatures_boost::size(offsetted_rings) > 0);
            ...
            bool decrease_segment_index_by_one)
    {
        ...
        piece pc;
        pc.type = type;
        pc.index = static_cast<signed_size_type>(geofeatures_boost::size(m_pieces));
        ...
        std::size_t const n = geofeatures_boost::size(offsetted_rings.back());
    ...
    inline void init_rescale_piece(piece& pc, std::size_t helper_points_size)
    {
        ...
        // Add rescaled offsetted segments
        {
            ...
            typedef typename geofeatures_boost::range_iterator<const buffered_ring<Ring> >::type it_type;
            for (it_type it = geofeatures_boost::begin(ring) + pc.first_seg_id.segment_index;
                it != geofeatures_boost::begin(ring) + pc.last_segment_index;
                ++it)
            {
                robust_point_type point;
    ...
    inline void sectionalize(piece& pc)
    {
        ...
        <
            point_type,
            geofeatures_boost::mpl::vector_c<std::size_t, 0, 1> // x,y dimension
        > sectionalizer;
        ...
        sectionalizer::apply(monotonic_sections,
            geofeatures_boost::begin(ring) + pc.first_seg_id.segment_index,
            geofeatures_boost::begin(ring) + pc.last_segment_index,
            m_robust_policy,
            ring_id, 10);
    }
    ...
    {
        BOOST_GEOMETRY_ASSERT(geofeatures_boost::size(range) != 0u);

        typename Range::const_iterator it = geofeatures_boost::begin(range);
        ...
        for (++it; it != geofeatures_boost::end(range); ++it)
        {
            ...
            bool decrease_segment_index_by_one)
    {
        ...
        if (geofeatures_boost::size(range) > 0u)
        {
    ...
    {
        BOOST_GEOMETRY_ASSERT(geofeatures_boost::size(range) >= 2u);
    ...
    inline void discard_nonintersecting_deflated_rings()
    {
        for(typename buffered_ring_collection<buffered_ring<Ring> >::iterator it
            = geofeatures_boost::begin(offsetted_rings);
            it != geofeatures_boost::end(offsetted_rings);
            ++it)
        {
            buffered_ring<Ring>& ring = *it;
            if (! ring.has_intersections()
                && geofeatures_boost::size(ring) > 0u
...
//        m_turns.erase
//            (
//                std::remove_if(geofeatures_boost::begin(m_turns), geofeatures_boost::end(m_turns),
//                                redundant_turn()),
//                geofeatures_boost::end(m_turns)
//            );
    ...
    inline void reverse()
    {
        for(typename buffered_ring_collection<buffered_ring<Ring> >::iterator it = geofeatures_boost::begin(offsetted_rings);
            it != geofeatures_boost::end(offsetted_rings);
            ++it)
        {
        ...
        for (typename geofeatures_boost::range_iterator<buffered_ring_collection<Ring> >::type
                it = geofeatures_boost::begin(traversed_rings);
                it != geofeatures_boost::end(traversed_rings);
                ++it)
        {
            std::reverse(it->begin(), it->end());
        }
    ...
    inline OutputIterator assign(OutputIterator out) const
    {
        ...
        for(typename buffered_ring_collection<buffered_ring<Ring> >::const_iterator it = geofeatures_boost::begin(offsetted_rings);
            it != geofeatures_boost::end(offsetted_rings);
            ++it, ++index)
        {
            if (! it->has_intersections()
        ...
        index = 0;
        for (typename geofeatures_boost::range_iterator<buffered_ring_collection<Ring> const>::type
                it = geofeatures_boost::begin(traversed_rings);
                it != geofeatures_boost::end(traversed_rings);
                ++it, ++index)
        {
