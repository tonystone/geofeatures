...
{
    PointOut point_out;
    for(typename geofeatures_boost::range_iterator<Range const>::type
        it = geofeatures_boost::begin(range);
...
struct transform_polygon
{
                ...
                Strategy const& strategy)
    {
        ...
        typename detail::interior_iterator<Polygon1 const>::type
            it1 = geofeatures_boost::begin(rings1);
        typename detail::interior_iterator<Polygon2>::type
            it2 = geofeatures_boost::begin(rings2);
        for ( ; it1 != geofeatures_boost::end(rings1); ++it1, ++it2)
        {
...
struct transform<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>, Geometry2>
{
    template <typename Strategy>
    ...
    {
        return geofeatures_boost::apply_visitor(visitor<Strategy>(geometry2, strategy), geometry1);
    }
};
