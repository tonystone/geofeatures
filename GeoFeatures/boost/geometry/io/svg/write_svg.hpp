...
struct svg_range
{
        ...
        Range const& range, std::string const& style, int )
    {
        ...
        for (iterator it = geofeatures_boost::begin(range);
...
struct svg_poly
{
        ...
        Polygon const& polygon, std::string const& style, int )
    {
        ...
        typedef typename geofeatures_boost::range_iterator<ring_type const>::type iterator_type;

        bool first = true;
        os << "<g fill-rule=\"evenodd\"><path d=\"";

        ring_type const& ring = geometry::exterior_ring(polygon);
        for (iterator_type it = geofeatures_boost::begin(ring);
            it != geofeatures_boost::end(ring);
            ++it, first = false)
        {
            os << (first ? "M" : " L") << " "
                << geometry::get<0>(*it)
        ...
        // Inner rings:
        {
            ...
            for (typename detail::interior_iterator<Polygon const>::type
                    rit = geofeatures_boost::begin(rings); rit != geofeatures_boost::end(rings); ++rit)
            {
                first = true;
                for (typename detail::interior_ring_iterator<Polygon const>::type
                        it = geofeatures_boost::begin(*rit); it != geofeatures_boost::end(*rit);
                    ++it, first = false)
                {
                    os << (first ? "M" : " L") << " "
                        << geometry::get<0>(*it)
