...
struct multiplicable_integral
{
    ...
    BOOST_STATIC_ASSERT(geofeatures_boost::is_integral<T>::value);

    static const std::size_t bits = CHAR_BIT * sizeof(T);
    static const std::size_t half_bits = bits / 2;
    typedef typename geofeatures_boost::make_unsigned<T>::type unsigned_type;
...
public :
                ...
                Segment const& c, Point const& fallback_point)
    {
        ...
        geofeatures_boost::ignore_unused(result1, result2, result3, result4, result5);
        //return result;
#endif
