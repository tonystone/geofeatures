// ----------------------------------------------------------------------------
// free_funcs.hpp :  implementation of the free functions of geofeatures_boost::format
// ----------------------------------------------------------------------------
...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost {
    ...
    namespace io {
         using ::geofeatures_boost::str; // keep compatibility with when it was defined in this N.S.
    }   // - namespace io
    ...
    {
        typedef geofeatures_boost::basic_format<Ch, Tr, Alloc>   format_t;
        if(f.items_.size()==0) 
            os << f.prefix_;
        else {
            if(f.cur_arg_ < f.num_args_)
                if( f.exceptions() & io::too_few_args_bit )
                    ...
                    geofeatures_boost::throw_exception(io::too_few_args(f.cur_arg_, f.num_args_)); 
