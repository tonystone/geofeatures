...
public:
    ...
    {
      if (x.count())
        save_impl(x.address(), geofeatures_boost::mpi::get_mpi_datatype(*x.address()), x.count());
    }
    ...
    MPI_Datatype get_mpi_datatype()
    {
      if (!is_committed)
      {
       ...
       BOOST_MPI_CHECK_RESULT(MPI_Type_create_struct,
                    (
                      addresses.size(),
                      geofeatures_boost::serialization::detail::get_data(lengths),
                      geofeatures_boost::serialization::detail::get_data(addresses),
                      geofeatures_boost::serialization::detail::get_data(types),
                      &datatype_
                    ));
#else
    ...
    void save(const T & t)
    {
        save_impl(&t, geofeatures_boost::mpi::get_mpi_datatype(t), 1);
    }
