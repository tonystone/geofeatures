       ...
       int root)
{
  if (comm.rank() == root) {
    ...
    ::geofeatures_boost::mpi::gather(comm, in_value, &out_values[0], root);
  } else {
    ::geofeatures_boost::mpi::gather(comm, in_value, root);
  }
}
       ...
       std::vector<T>& out_values, int root)
{
  if (comm.rank() == root) {
    ...
    ::geofeatures_boost::mpi::gather(comm, in_values, n, &out_values[0], root);
  } 
  else
    ::geofeatures_boost::mpi::gather(comm, in_values, n, root);
}
