...
public:
   ...
   */
  packed_oarchive( MPI_Comm const & comm, buffer_type & b, unsigned int flags = geofeatures_boost::archive::no_header)
         : oprimitive(b,comm),
   ...
   *  default flags.
   */
  packed_oarchive ( MPI_Comm const & comm, unsigned int flags =  geofeatures_boost::archive::no_header)
  ...
  void save_override(const archive::class_id_type & t){
    const geofeatures_boost::int_least16_t x = t;
    * this->This() << x;
  }

  void save_override(const archive::version_type & t){
    const geofeatures_boost::int_least8_t x = t;
    * this->This() << x;
  }
private:
...
// required by export
BOOST_SERIALIZATION_REGISTER_ARCHIVE(geofeatures_boost::mpi::packed_oarchive)
BOOST_SERIALIZATION_USE_ARRAY_OPTIMIZATION(geofeatures_boost::mpi::packed_oarchive)
