...
// TODO: move the above condition into the exit_watcher?
                   ...
                   OtherBoundaryChecker const& /*other_boundary_checker*/)
        {
                ...
                || m_degenerated_turn_ptr )
            {
                ...
                if ( turn_ptr )
                {
                    ...
                    //BOOST_GEOMETRY_ASSERT(!geofeatures_boost::empty(sub_range(geometry, prev_seg_id)));
                                ...
                                bool first_in_range)
        {
            ...
            {
                // valid, point-sized
                if ( geofeatures_boost::size(ls2_ref) == 2 )
                {
                    ...
                    m_degenerated_turn_ptr = geofeatures_boost::addressof(turn);
                }
            }
            else if ( turn.operations[op_id].position == overlay::position_back )
            {
                ...
                if ( geofeatures_boost::size(ls2_ref) == 2 )
                {
                    ...
                    if ( first_in_range )
                    {
                        //BOOST_GEOMETRY_ASSERT(!geofeatures_boost::empty(ls1_ref));
                        bool const front_b = is_endpoint_on_boundary<boundary_front>(
                   ...
                   && turn.operations[other_op_id].position == overlay::position_middle )
            {
                ...
                bool const is_point1 = geofeatures_boost::size(ls1_ref) == 2
                                    && equals::equals_point_point(range::front(ls1_ref), range::back(ls1_ref));
                bool const is_point2 = geofeatures_boost::size(ls2_ref) == 2
