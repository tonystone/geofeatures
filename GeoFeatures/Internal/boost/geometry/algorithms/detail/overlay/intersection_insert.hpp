...
struct intersection_linestring_linestring_point
{
            ...
            Strategy const& )
    {
        ...
        for (typename geofeatures_boost::range_iterator<std::deque<turn_info> const>::type
            it = geofeatures_boost::begin(turns); it != geofeatures_boost::end(turns); ++it)
        {
            PointOut p;
...
struct intersection_of_linestring_with_areal
{
    ...
    static inline OutputIterator apply(LineString const& linestring, Areal const& areal,
            ...
            Strategy const& )
    {
        if (geofeatures_boost::size(linestring) == 0)
        {
            return out;
        }
...
inline OutputIterator insert(Geometry1 const& geometry1,
            ...
            Strategy const& strategy)
{
    return geofeatures_boost::mpl::if_c
    <
