...
struct buffer<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    template <typename Distance, typename GeometryOut>
    ...
    static inline void apply(
        geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const& geometry,
        Distance const& distance,
        Distance const& chord_length,
        GeometryOut& out
    )
    {
        geofeatures_boost::apply_visitor(visitor<Distance, GeometryOut>(distance, chord_length, out), geometry);
    }
};
...
{
    typedef typename geofeatures_boost::range_value<MultiPolygon>::type polygon_type;
    ...
    rescale_policy_type rescale_policy
            = geofeatures_boost::geometry::get_rescale_policy<rescale_policy_type>(box);
