...
// Silence warning C4512: 'geofeatures_boost::geometry::wkt_manipulator<Geometry>' : assignment operator could not be generated
#if defined(_MSC_VER)
#pragma warning(push)  
...
struct wkt_range
{
                ...
                Range const& range, bool force_closed)
    {
        ...
        iterator_type begin = geofeatures_boost::begin(range);
        ...
        if (force_closed 
            && geofeatures_boost::size(range) > 1
...
struct wkt_multi
{
                ...
                Multi const& geometry)
    {
        ...
        for (typename geofeatures_boost::range_iterator<Multi const>::type
                    it = geofeatures_boost::begin(geometry);
            it != geofeatures_boost::end(geometry);
            ++it)
        {
            if (it != geofeatures_boost::begin(geometry))
            {
                os << ",";
            }
...
struct wkt_segment
{
                ...
                Segment const& segment)
    {
        ...
        typedef geofeatures_boost::array<point_type, 2> sequence;
...
struct devarianted_wkt<variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    ...
    {
        geofeatures_boost::apply_visitor(visitor<OutputStream>(os), geometry);
    }
};
