...
struct destroy_elements
{
    ...
    {
        apply(geofeatures_boost::begin(elements), geofeatures_boost::end(elements), allocators);
    }
...
private:
    ...
    inline static void apply_dispatch(It first, It last, Allocators & allocators,
                                      geofeatures_boost::mpl::bool_<false> const& /*is_range_of_values*/)
    {
        typedef rtree::subtree_destroyer<Value, Options, Translator, Box, Allocators> subtree_destroyer;
    ...
    template <typename It>
    inline static void apply_dispatch(It /*first*/, It /*last*/, Allocators & /*allocators*/,
                                      geofeatures_boost::mpl::bool_<true> const& /*is_range_of_values*/)
    {}
};
...
void move_from_back(Container & container, Iterator it)
{
    ...
    if ( it != back_it )
    {
        *it = geofeatures_boost::move(*back_it);                                                             // MAY THROW (copy)
    }
}
