...
struct closing_iterator
    ...
    <
        ...
        typename geofeatures_boost::range_value<Range>::type const,
        geofeatures_boost::random_access_traversal_tag
    >
{
    typedef typename geofeatures_boost::range_difference<Range>::type difference_type;

    /// Constructor including the range it is based on
    explicit inline closing_iterator(Range& range)
        : m_range(&range)
        , m_iterator(geofeatures_boost::begin(range))
        , m_end(geofeatures_boost::end(range))
        , m_size(static_cast<difference_type>(geofeatures_boost::size(range)))
        , m_index(0)
    {}
    ...
    explicit inline closing_iterator(Range& range, bool)
        : m_range(&range)
        , m_iterator(geofeatures_boost::end(range))
        , m_end(geofeatures_boost::end(range))
        , m_size(static_cast<difference_type>(geofeatures_boost::size(range)))
...
private:
    ...
    inline typename geofeatures_boost::range_value<Range>::type const& dereference() const
    {
        return *m_iterator;
    }
    ...
    inline void update_iterator()
    {
        this->m_iterator = m_index <= m_size
            ? geofeatures_boost::begin(*m_range) + (m_index % m_size)
            : geofeatures_boost::end(*m_range)
            ;
    }

    Range* m_range;
    typename geofeatures_boost::range_iterator<Range>::type m_iterator;
    typename geofeatures_boost::range_iterator<Range>::type m_end;
