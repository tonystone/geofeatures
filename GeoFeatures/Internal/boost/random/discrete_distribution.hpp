...
public:
    ...
    public:
        ...
        explicit param_type(const Range& range)
          : _probabilities(geofeatures_boost::begin(range), geofeatures_boost::end(range))
        {
            normalize();
        }
    ...
    {
        init(geofeatures_boost::begin(range), geofeatures_boost::end(range));
    }
    /**
...
private:
    ...
    typedef typename detail::select_alias_table<
        (::geofeatures_boost::is_integral<WeightType>::value)
