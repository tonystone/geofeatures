...
public:
    ...
    RealType operator()(URNG& eng) 
    {
        using std::sqrt;
        if (_param.k() > 1) {
            geofeatures_boost::random::normal_distribution<RealType> n_dist;
            geofeatures_boost::random::chi_squared_distribution<RealType> c_dist(_param.k() - RealType(1));
            ...
            geofeatures_boost::random::poisson_distribution<> p_dist(_param.lambda()/RealType(2));
            geofeatures_boost::random::poisson_distribution<>::result_type _p = p_dist(eng);
            geofeatures_boost::random::chi_squared_distribution<RealType> c_dist(_param.k() + RealType(2)*_p);
            return c_dist(eng);
        }
    }
