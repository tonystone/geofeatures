...
{
   typedef geofeatures_boost::container::map<geofeatures_boost::container::string, ref_count_ptr> map_type;

   public:
   windows_semaphore_based_map()
   {
      map_type *m = new map_type;
      geofeatures_boost::uint32_t initial_count = 0;
      geofeatures_boost::uint32_t max_count = 0;
      ...
      if(sizeof(void*) == sizeof(geofeatures_boost::uint32_t)){
         ...
         {
            void *addr;
            geofeatures_boost::uint32_t addr_uint32;
         } caster;
         caster.addr = m;
         //memory is at least 4 byte aligned in windows
         BOOST_ASSERT((caster.addr_uint32 & geofeatures_boost::uint32_t(3)) == 0);
         max_count = caster.addr_uint32 >> 2;
      }
      else if(sizeof(void*) == sizeof(geofeatures_boost::uint64_t)){
         //Relying in UB with a cast through union, but all known windows compilers
         //accept this (C11 accepts this).
         union caster_union
         {
            void *addr;
            geofeatures_boost::uint64_t addr_uint64;
         } caster;
         caster.addr = m;
         ...
         caster.addr = m;
         BOOST_ASSERT((caster.addr_uint64 & geofeatures_boost::uint64_t(3)) == 0);
         max_count = geofeatures_boost::uint32_t(caster.addr_uint64 >> 32);
         initial_count = geofeatures_boost::uint32_t(caster.addr_uint64);
         initial_count = initial_count/4;
         //Make sure top two bits are zero
         BOOST_ASSERT((max_count & geofeatures_boost::uint32_t(0xC0000000)) == 0);
         //Set quasi-top bit
         max_count |= geofeatures_boost::uint32_t(0x40000000);
      }
      bool created = false;
   ...
   map_type &get_map_unlocked()
   {
      if(sizeof(void*) == sizeof(geofeatures_boost::uint32_t)){
         ...
         geofeatures_boost::uint32_t max_count(m_sem_map.limit()), initial_count(m_sem_map.value());
         //Clear quasi-top bit
         max_count &= geofeatures_boost::uint32_t(0xBFFFFFFF);
         caster.addr_uint64 = max_count;
         caster.addr_uint64 =  caster.addr_uint64 << 32;
         caster.addr_uint64 |= geofeatures_boost::uint64_t(initial_count) << 2;
         return *static_cast<map_type*>(caster.addr);
      }
   }
   ...
   ref_count_ptr *find(const char *name)
   {
      ...
      map_type::iterator it = map.find(geofeatures_boost::container::string(name));
      if(it != map.end()){
         return &it->second;
      }
      else{
   ...
   ref_count_ptr * insert(const char *name, const ref_count_ptr &ref)
   {
      scoped_lock<winapi_mutex_wrapper> lck(m_mtx_lock);
      map_type &map = this->get_map_unlocked();
      map_type::iterator it = map.insert(map_type::value_type(geofeatures_boost::container::string(name), ref)).first;
      return &it->second;
   }
   ...
   bool erase(const char *name)
   {
      scoped_lock<winapi_mutex_wrapper> lck(m_mtx_lock);
      map_type &map = this->get_map_unlocked();
      return map.erase(geofeatures_boost::container::string(name)) != 0;
   }
