...
#define BOOST_TYPEOF_TEMPLATE_PARAM_ENCODE(This, n)\
   typedef typename geofeatures_boost::type_of::encode_template<BOOST_PP_CAT(V, n),\
...
#define BOOST_TYPEOF_TEMPLATE_PARAM_DECODE(This, n)\
   typedef geofeatures_boost::type_of::decode_template< BOOST_PP_CAT(iter, n) > BOOST_PP_CAT(d, n);\
...
#define BOOST_TYPEOF_REGISTER_TYPE_FOR_TEMPLATE_TEMPLATE(Name,Params,ID)\
    ...
    struct encode_template_impl<V,Name<\
        ...
        P)> >\
        : geofeatures_boost::type_of::push_back<V, geofeatures_boost::mpl::size_t<ID> >\
    {\
    };\
    template<class Iter> struct decode_template_impl<geofeatures_boost::mpl::size_t<ID>, Iter>\
    {\
