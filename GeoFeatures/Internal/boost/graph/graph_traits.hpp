...
#define BOOST_GRAPH_MEMBER_OR_VOID(name) \
      ...
      template <typename T> struct BOOST_JOIN(get_opt_member_, name): \
        geofeatures_boost::mpl::eval_if_c< \
          BOOST_JOIN(has_, name)<T>::value, \
          BOOST_JOIN(get_member_, name)<T>, \
          geofeatures_boost::mpl::identity<void> > \
        {};
...
#define BOOST_GRAPH_SEQUENTIAL_TRAITS_DEFINES_DISTRIBUTED_TAGS // Disable these from external versions of PBGL
    ...
    typedef geofeatures_boost::forward_traversal_tag multi_pass_input_iterator_tag;
    ...
    struct graph_property_type
      : geofeatures_boost::mpl::eval_if<detail::has_graph_property_type<G>,
                            detail::get_graph_property_type<G>,
                            no_property> {};
    template <typename G>
    struct edge_property_type
      : geofeatures_boost::mpl::eval_if<detail::has_edge_property_type<G>,
                            detail::get_edge_property_type<G>,
                            no_property> {};
    template <typename G>
    struct vertex_property_type
      : geofeatures_boost::mpl::eval_if<detail::has_vertex_property_type<G>,
