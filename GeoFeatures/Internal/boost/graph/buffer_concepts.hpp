...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost {
  ...
  BOOST_concept(Buffer, (B))
  {
    ...
    BOOST_CONCEPT_USAGE(Buffer) {
      typedef typename geofeatures_boost::add_reference<value_type>::type reference;
      ...
      buf.pop();
      reference t = buf.top();
      geofeatures_boost::ignore_unused_variable_warning(t);
    }
    
    void const_constraints(const B& cbuf) {
      typedef typename geofeatures_boost::add_const<typename geofeatures_boost::remove_reference<value_type>::type>::type& const_reference;
    ...
    : UpdatableQueue<Q>
  {
    ...
    BOOST_CONCEPT_USAGE(KeyedUpdatableQueue) {
      BOOST_CONCEPT_ASSERT((geofeatures_boost::ReadWritePropertyMapConcept<key_map, typename Buffer<Q>::value_type>));
    }
