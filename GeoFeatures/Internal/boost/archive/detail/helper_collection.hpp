...
class helper_collection
{
    ...
    typedef std::pair<
        const void *,
        geofeatures_boost::shared_ptr<void>
    > helper_value_type;
    template<class T>
    geofeatures_boost::shared_ptr<void> make_helper_ptr(){
        // use geofeatures_boost::shared_ptr rather than std::shared_ptr to maintain
        // c++03 compatibility
        return geofeatures_boost::make_shared<T>();
    }
