...
struct object_manager_get_pytype<true>
{
...
#ifndef BOOST_PYTHON_NO_PY_SIGNATURES
      typedef geofeatures_boost::mpl::bool_<is_handle<T>::value> is_t_handle;
      typedef geofeatures_boost::detail::indirect_traits::is_reference_to_const<T> is_t_const;
  ...
  struct shared_ptr_to_python_value
  {
      ...
      PyObject* operator()(argument_type) const;
#ifndef BOOST_PYTHON_NO_PY_SIGNATURES
      PyTypeObject const* get_pytype() const {return get_pytype((geofeatures_boost::type<argument_type>*)0);}
#endif 
      // This information helps make_getter() decide whether to try to
      // return an internal reference or not. I don't like it much,
      // but it will have to serve for now.
...
#ifndef BOOST_PYTHON_NO_PY_SIGNATURES
      template <class U>
      PyTypeObject const* get_pytype(geofeatures_boost::type<shared_ptr<U> &> *) const {return converter::registered<U>::converters.to_python_target_type();}
      template <class U>
      PyTypeObject const* get_pytype(geofeatures_boost::type<const shared_ptr<U> &> *) const {return converter::registered<U>::converters.to_python_target_type();}
#endif
  };
}
