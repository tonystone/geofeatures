...
#   define DEDUCE_RESULT_TYPE( oper )                                   \
    decltype(geofeatures_boost::declval<Lhs>() oper geofeatures_boost::declval<Rhs>() ) optype; \
    typedef typename geofeatures_boost::remove_reference<optype>::type              \
/**/
#else
...
#define ADD_OP_SUPPORT( oper, name, _ )                         \
    template<typename T>                                        \
    binary_expr<ExprType,typename geofeatures_boost::decay<T const>::type,  \
        op::name<ValType,typename geofeatures_boost::decay<T const>::type> > \
    operator oper( T const& rhs ) const                         \
    {                                                           \
        typedef typename geofeatures_boost::decay<T const>::type Rhs;       \
