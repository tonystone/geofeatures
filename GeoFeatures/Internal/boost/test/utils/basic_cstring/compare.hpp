...
template<class CharT>
inline bool
operator <( geofeatures_boost::unit_test::basic_cstring<CharT> const& x,
            geofeatures_boost::unit_test::basic_cstring<CharT> const& y )
{
    typedef typename geofeatures_boost::unit_test::basic_cstring<CharT>::traits_type traits_type;
    return x.size() != y.size()
            ? x.size() < y.size()
