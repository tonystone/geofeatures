...
extract( geofeatures_boost::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = geofeatures_boost::get_error_info<ErrorInfo>( *ex );
...
static void
report_error( execution_exception::error_code ec, geofeatures_boost::exception const* be, char const* format, va_list* args )
{
...
static void
report_error( execution_exception::error_code ec, geofeatures_boost::exception const* be, char const* format, ... )
{
    va_list args;
...
// ************************************************************************** //
// **************    geofeatures_boost::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************         geofeatures_boost::detail::signal_action         ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************        geofeatures_boost::detail::signal_handler         ************** //
// ************************************************************************** //
...
int
execution_monitor::catch_signals( geofeatures_boost::function<int ()> const& F )
{
...
int
execution_monitor::execute( geofeatures_boost::function<int ()> const& F )
{
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<geofeatures_boost::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
#undef CATCH_AND_REPORT_STD_EXCEPTION
    ...
    catch( geofeatures_boost::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
#ifdef BOOST_NO_TYPEID
                              "unknown geofeatures_boost::exception" ); }
#else
                              typeid(ex).name()          ); }
#endif
...
struct forward {
    explicit    forward( geofeatures_boost::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    geofeatures_boost::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( geofeatures_boost::function<void ()> const& F )
{
...
{
    geofeatures_boost::ignore_unused(mask);
