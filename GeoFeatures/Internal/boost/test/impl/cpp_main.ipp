...
prg_exec_monitor_main( int (*cpp_main)( int argc, char* argv[] ), int argc, char* argv[] )
{
    ...
    BOOST_TEST_IMPL_TRY {
        geofeatures_boost::unit_test::const_string p( std::getenv( "BOOST_TEST_CATCH_SYSTEM_ERRORS" ) );
        ::geofeatures_boost::execution_monitor ex_mon;
        ...
        if( result == 0 )
            result = ::geofeatures_boost::exit_success;
        else if( result != ::geofeatures_boost::exit_success ) {
            std::cout << "\n**** error return code: " << result << std::endl;
            result = ::geofeatures_boost::exit_failure;
        }
    }
    BOOST_TEST_IMPL_CATCH( ::geofeatures_boost::execution_exception, exex ) {
        std::cout << "\n**** exception(" << exex.code() << "): " << exex.what() << std::endl;
        result = ::geofeatures_boost::exit_exception_failure;
    }
    BOOST_TEST_IMPL_CATCH( ::geofeatures_boost::system_error, ex ) {
    ...
    if( result != ::geofeatures_boost::exit_success ) {
        ...
        ::geofeatures_boost::unit_test::const_string p( std::getenv( "BOOST_PRG_MON_CONFIRM" ) );
        if( p != "no" ) {
...
{
    return ::geofeatures_boost::prg_exec_monitor_main( &cpp_main, argc, argv );
}
