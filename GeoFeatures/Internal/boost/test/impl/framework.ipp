...
private:
    bool            filter_unit( test_unit const& tu )
    {
        ...
        // look for match
        return std::find_if( filters.begin(), filters.end(), bind( &component::pass, _1, geofeatures_boost::ref(tu) ) ) != filters.end();
    }
...
public:
    ...
    execution_result execute_test_tree( test_unit_id tu_id, unsigned timeout = 0 )
    {
        ...
        BOOST_TEST_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
            result = unit_test_monitor.execute_and_translate( geofeatures_boost::bind( &test_unit_fixture::setup, F ) );
        ...
        if( result == unit_test_monitor_t::test_ok ) {
            ...
            geofeatures_boost::timer tu_timer;
        ...
        if( !unit_test_monitor.is_critical_error( result ) ) {
            ...
            BOOST_TEST_REVERSE_FOREACH( test_unit_fixture_ptr, F, tu.p_fixtures.get() ) {
                result = (std::min)( result, unit_test_monitor.execute_and_translate( geofeatures_boost::bind( &test_unit_fixture::teardown, F ), 0 ) );
    ...
    geofeatures_boost::execution_monitor m_aux_em;
};
...
init( init_unit_test_func init_func, int argc, char* argv[] )
{
    ...
    BOOST_TEST_IMPL_TRY {
        s_frk_state().m_aux_em.vexecute( geofeatures_boost::bind( &impl::invoke_init_func, init_func ) );
    }
...
int
add_context( ::geofeatures_boost::unit_test::lazy_ostream const& context_descr, bool sticky )
{
...
run( test_unit_id id, bool continue_test )
{
    ...
    if( call_start_finish ) {
        BOOST_TEST_FOREACH( test_observer*, to, impl::s_frk_state().m_observers ) {
            BOOST_TEST_IMPL_TRY {
                impl::s_frk_state().m_aux_em.vexecute( geofeatures_boost::bind( &test_observer::test_start, to, tcc.p_count ) );
            }
            BOOST_TEST_IMPL_CATCH( execution_exception, ex ) {
                BOOST_TEST_SETUP_ASSERT( false, ex.what() );
            }
