...
*/
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost { namespace polygon { namespace detail {
...
struct minkowski_offset {
  ...
  polygon_set_data<T>::resize(coordinate_type resizing, bool corner_fill_arc, unsigned int num_circle_segments) {
    using namespace ::geofeatures_boost::polygon::operators;
    if(!corner_fill_arc) {
      if(resizing < 0)
    ...
    extents(rect);
    if(resizing < 0) {
      ::geofeatures_boost::polygon::bloat(rect, 10);
