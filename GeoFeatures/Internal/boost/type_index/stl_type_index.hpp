...
/// \brief Contains geofeatures_boost::typeindex::stl_type_index class.
///
/// geofeatures_boost::typeindex::stl_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
...
/// is defined geofeatures_boost::typeindex::ctti is usually used instead of geofeatures_boost::typeindex::stl_type_index.
...
inline std::string stl_type_index::pretty_name() const {
    static const char cvr_saver_name[] = "geofeatures_boost::typeindex::detail::cvr_saver<";
    ...
    const geofeatures_boost::core::scoped_demangled_name demangled_name(data_->name());

    const char* begin = demangled_name.get();
    if (!begin) {
        geofeatures_boost::throw_exception(std::runtime_error("Type name demangling failed"));
    }
...
#else
    return geofeatures_boost::hash_range(raw_name(), raw_name() + std::strlen(raw_name()));
#endif
}
...
inline stl_type_index stl_type_index::type_id() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME geofeatures_boost::remove_reference<T>::type no_ref_t;
    typedef BOOST_DEDUCED_TYPENAME geofeatures_boost::remove_cv<no_ref_t>::type no_cvr_prefinal_t;
    ...
    #  if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) \
        ...
        typedef BOOST_DEDUCED_TYPENAME geofeatures_boost::mpl::if_<
            geofeatures_boost::is_signed<no_cvr_prefinal_t>,
            geofeatures_boost::make_signed<no_cvr_prefinal_t>,
            geofeatures_boost::mpl::identity<no_cvr_prefinal_t>
...
inline stl_type_index stl_type_index::type_id_with_cvr() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME geofeatures_boost::mpl::if_<
        geofeatures_boost::mpl::or_<geofeatures_boost::is_reference<T>, geofeatures_boost::is_const<T>, geofeatures_boost::is_volatile<T> >,
        detail::cvr_saver<T>,
        T
    >::type type;
