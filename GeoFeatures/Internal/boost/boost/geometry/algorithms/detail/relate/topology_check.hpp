...
struct topology_check<Linestring, linestring_tag>
{
    ...
    {
        std::size_t count = geofeatures_boost::size(ls);
...
struct topology_check<MultiLinestring, multi_linestring_tag>
{
    ...
    void init(MultiLinestring const& mls, OddCounter const& odd_counter)
    {
        ...
        endpoints.reserve(geofeatures_boost::size(mls) * 2);

        typedef typename geofeatures_boost::range_iterator<MultiLinestring const>::type ls_iterator;
        for ( ls_iterator it = geofeatures_boost::begin(mls) ; it != geofeatures_boost::end(mls) ; ++it )
        {
            std::size_t count = geofeatures_boost::size(*it);
