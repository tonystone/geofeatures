                ...
                std::string const& header)
{
    ...
    if (geofeatures_boost::contains(header, "Finished"))
    {
        std::cout << std::endl;
    }
}
...
inline void set_visited_for_continue(Info& info, Turn const& turn)
{
    ...
    {
        for (typename geofeatures_boost::range_iterator
            <
                typename Info::container_type
            >::type it = geofeatures_boost::begin(info.operations);
            it != geofeatures_boost::end(info.operations);
            ++it)
        {
            if (it->visited.none())
            {
...
inline bool assign_next_ip(G1 const& g1, G2 const& g2,
            Turns& turns,
            typename geofeatures_boost::range_iterator<Turns>::type& ip,
            ...
            RobustPolicy const& robust_policy)
{
    ...
    if (info.enriched.next_ip_index < 0)
    {
        ...
        seg_id = info.seg_id;
        ip = geofeatures_boost::begin(turns) + info.enriched.travels_to_ip_index;
    }
    else
    {
        ip = geofeatures_boost::begin(turns) + info.enriched.next_ip_index;
        seg_id = info.seg_id;
    }
            ...
            Iterator& selected)
{
    ...
    bool has_tp = false;
    selected = geofeatures_boost::end(turn.operations);
    for (Iterator it = geofeatures_boost::begin(turn.operations);
        it != geofeatures_boost::end(turn.operations);
        ++it)
    {
...
public :
    ...
    {
        typedef typename geofeatures_boost::range_value<Rings>::type ring_type;
        typedef typename geofeatures_boost::range_iterator<Turns>::type turn_iterator;
        typedef typename geofeatures_boost::range_value<Turns>::type turn_type;
        ...
        std::size_t size_at_start = geofeatures_boost::size(rings);
        ...
        do
        {
            ...
            for (turn_iterator it = geofeatures_boost::begin(turns);
                state.good() && it != geofeatures_boost::end(turns);
                ++it)
            {
                // Skip discarded ones
                if (! (it->discarded || ! it->selectable_start || it->blocked()))
                {
                    for (turn_operation_iterator_type iit = geofeatures_boost::begin(it->operations);
                        state.good() && iit != geofeatures_boost::end(it->operations);
                        ++iit)
                    {
                        if (iit->visited.none()
                            ...
                            )
                        {
                            ...
                            else
                            {
                                ...
                                typename geofeatures_boost::range_size<Turns>::type i = 0;
