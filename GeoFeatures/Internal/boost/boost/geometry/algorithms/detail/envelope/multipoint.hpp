...
private:
    ...
    static inline void analyze_point_coordinates(MultiPoint const& multipoint,
                                                 ...
                                                 OutputIterator oit)
    {
        typedef typename geofeatures_boost::range_value<MultiPoint>::type point_type;
        ...
        for (iterator_type it = geofeatures_boost::begin(multipoint);
             it != geofeatures_boost::end(multipoint);
             ++it)
        {
                                    ...
                                    Value& max_gap_right)
    {
        ...
        iterator_type it1 = geofeatures_boost::begin(sorted_range), it2 = it1;
        ++it2;
        max_gap_left = geometry::get<0>(*it1);
        max_gap_right = geometry::get<0>(*it2);

        Value max_gap = max_gap_right - max_gap_left;
        for (++it1, ++it2; it2 != geofeatures_boost::end(sorted_range); ++it1, ++it2)
        {
                                              ...
                                              CoordinateType& lon_max)
    {
        ...
        std::pair<iterator_type, iterator_type> min_max_longitudes
            = geofeatures_boost::minmax_element(geofeatures_boost::begin(range),
        ...
        {
            std::sort(geofeatures_boost::begin(range), geofeatures_boost::end(range), lon_less);
                                             ...
                                             CoordinateType& lat_max)
    {
        ...
        else
        {
            std::pair<Iterator, Iterator> min_max_latitudes
                = geofeatures_boost::minmax_element(first, last, lat_less);
...
public:
    ...
    static inline void apply(MultiPoint const& multipoint, Box& mbr)
    {
        ...
        if (geofeatures_boost::empty(multipoint))
        {
        ...
        if (geofeatures_boost::size(multipoint) == 1)
        {
            return dispatch::envelope
                <
                    typename geofeatures_boost::range_value<MultiPoint>::type
        ...
        iterator_type it = geofeatures_boost::begin(multipoint);
        envelope_one_point<2, dimension<Box>::value>::apply(*it, mbr);

        for (++it; it != geofeatures_boost::end(multipoint); ++it)
        {
