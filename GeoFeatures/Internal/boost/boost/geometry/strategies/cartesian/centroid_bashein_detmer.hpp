...
public :
            ...
            PointOfSegment const& p2, sums& state)
    {
        ...
        */
        ...
        calculation_type const x1 = geofeatures_boost::numeric_cast<calculation_type>(get<0>(p1));
        calculation_type const y1 = geofeatures_boost::numeric_cast<calculation_type>(get<1>(p1));
        calculation_type const x2 = geofeatures_boost::numeric_cast<calculation_type>(get<0>(p2));
        calculation_type const y2 = geofeatures_boost::numeric_cast<calculation_type>(get<1>(p2));
    ...
    static inline bool result(sums const& state, Point& centroid)
    {
        ...
        if (state.count > 0 && ! math::equals(state.sum_a2, zero))
        {
            ...
            if (geofeatures_boost::math::isfinite(a3))
            {
                ...
                set<0>(centroid,
                    geofeatures_boost::numeric_cast<coordinate_type>(state.sum_x / a3));
                set<1>(centroid,
                    geofeatures_boost::numeric_cast<coordinate_type>(state.sum_y / a3));
                return true;
            }
        }
