...
@implementation GFLineStringAbstract (Protected)
    ...
    - (climate::gf::LineString)cppLineStringWithGeoJSONCoordinates:(NSArray *)coordinates {
        ...
        try {
            ...
            for (NSArray * coordinate in coordinates) {
                linestring.push_back(climate::gf::Point([coordinate[0] doubleValue], [coordinate[1] doubleValue]));
            }
    ...
    - (NSArray *)geoJSONCoordinatesWithCPPLineString: (const climate::gf::LineString &) linestring  {
        ...
        try {
            for (climate::gf::LineString::vector::const_iterator it = linestring.begin();  it != linestring.end(); ++it ) {
    ...
    - (id <MKOverlay>)mkOverlayWithCPPLineString: (const climate::gf::LineString &) linestring {
        ...
        try {
            ...
            for (climate::gf::LineString::vector::size_type i = 0; i < pointCount; i++) {
                const climate::gf::Point& point = linestring.at(i);
