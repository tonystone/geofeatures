...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost { namespace numeric { namespace ublas {
	...
	/// \brief Return a const pointer to the element of index i
	 ...
	 /// vector into a normal vector. It must first be assigned to another normal vector by any suitable means. Its memory footprint is constant.
	 /// \tparam T type of the objects stored in the vector: it can be anything even if most of the time, scalar types will be used like \c double or \c int. Complex types can be used, or even classes like geofeatures_boost::interval.
	 template<class T, class ALLOC>
	 class scalar_vector:
