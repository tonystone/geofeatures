    ...
    clear_bind (int argN) {
        ...
        if(argN<1 || argN > num_args_ || bound_.size()==0 || !bound_[argN-1] ) {
            if( exceptions() & io::out_of_range_bit)
                geofeatures_boost::throw_exception(io::out_of_range(argN, 1, num_args_+1 ) ); 
            else return *this;
        }
        bound_[argN-1]=false;
        clear();
    ...
    str () const {
        ...
        if( cur_arg_ < num_args_)
            if( exceptions() & io::too_few_args_bit )
                ...
                geofeatures_boost::throw_exception(io::too_few_args(cur_arg_, num_args_)); 
...
namespace detail {
    ...
    bind_arg_body (basic_format<Ch, Tr, Alloc>& self, int argN, const T& val) {
        ...
        if(argN<1 || argN > self.num_args_) {
            if( self.exceptions() & io::out_of_range_bit )
                geofeatures_boost::throw_exception(io::out_of_range(argN, 1, self.num_args_+1 ) );
            else return self;
        }
    ...
    modify_item_body (basic_format<Ch, Tr, Alloc>& self, int itemN, T manipulator) {
        ...
        if(itemN<1 || itemN > static_cast<signed int>(self.items_.size() )) {
            if( self.exceptions() & io::out_of_range_bit ) 
                geofeatures_boost::throw_exception(io::out_of_range(itemN, 1, static_cast<int>(self.items_.size()) ));
            else return self;
        }
