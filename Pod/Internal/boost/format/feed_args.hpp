...
// the trick is in "boost/format/msvc_disambiguater.hpp"
    ...
    void call_put_head(BOOST_IO_STD basic_ostream<Ch, Tr> & os, const void* x) {
        put_head(os, *(typename ::geofeatures_boost::remove_reference<T>::type*)x);
    }
    ...
    void distribute (basic_format<Ch,Tr, Alloc>& self, T x) {
        ...
        if(self.cur_arg_ >= self.num_args_)  {
            if( self.exceptions() & too_many_args_bit )
                geofeatures_boost::throw_exception(too_many_args(self.cur_arg_, self.num_args_)); 
            else return;
        }
        for(unsigned long i=0; i < self.items_.size(); ++i) {
            if(self.items_[i].argN_ == self.cur_arg_) {
                put<Ch, Tr, Alloc, T> (x, self.items_[i], self.items_[i].res_, 
                                self.buf_, geofeatures_boost::get_pointer(self.loc_) );
            }
        }
    }
