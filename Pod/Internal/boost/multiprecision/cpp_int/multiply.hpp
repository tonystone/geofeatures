      ...
      const signed_limb_type& val) BOOST_MP_NOEXCEPT_IF((is_non_throwing_cpp_int<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value))
{
   ...
   {
      eval_multiply(result, a, static_cast<limb_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(val)));
      result.negate();
   }
}
...
{
   typedef typename geofeatures_boost::multiprecision::detail::canonical<typename cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>::local_limb_type, cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::type canonical_type;
