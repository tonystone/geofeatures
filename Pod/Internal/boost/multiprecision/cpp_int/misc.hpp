...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost{ namespace multiprecision{ namespace backends{

template <class R, class CppInt>
void check_in_range(const CppInt& val, const mpl::int_<checked>&)
{
   typedef typename geofeatures_boost::multiprecision::detail::canonical<R, CppInt>::type cast_type;
   if(val.sign())
   {
   ...
   eval_convert_to(R* result, const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& backend) BOOST_MP_NOEXCEPT_IF((is_non_throwing_cpp_int<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value))
{
   ...
   *result = static_cast<R>(backend.limbs()[0]);
   ...
   if(backend.sign())
   {
      check_is_negative(geofeatures_boost::is_signed<R>());
      *result = negate_integer(*result, geofeatures_boost::is_signed<R>());
   }
}
   ...
   eval_lsb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   //
   unsigned result = geofeatures_boost::multiprecision::detail::find_lsb(a.limbs()[index]);
   ...
   eval_msb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   //
   return (a.size() - 1) * cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>::limb_bits + geofeatures_boost::multiprecision::detail::find_msb(a.limbs()[a.size() - 1]);
}
...
BOOST_MP_FORCEINLINE typename enable_if_c<is_signed<Integer>::value && !is_trivial_cpp_int<cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1> >::value, Integer>::type
   eval_integer_modulus(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& x, Integer val)
{
   return eval_integer_modulus(x, geofeatures_boost::multiprecision::detail::unsigned_abs(val));
}
...
inline limb_type integer_gcd_reduce(limb_type u, limb_type v)
{
   do
   {
      ...
      if(u == v)
         break;
      v -= u;
      v >>= geofeatures_boost::multiprecision::detail::find_lsb(v);
   } while(true);
   return u;
}
      ...
      limb_type v)
{
   ...
   unsigned vs = geofeatures_boost::multiprecision::detail::find_lsb(v);
   shift = (std::min)(us, vs);
...
{
   *result.limbs() = geofeatures_boost::integer::gcd(*a.limbs(), *b.limbs());
}
...
{
   *result.limbs() = geofeatures_boost::integer::lcm(*a.limbs(), *b.limbs());
   ...
   eval_convert_to(R* result, const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& val)
{
   ...
   if(val.isneg())
   {
      check_is_negative(mpl::bool_<geofeatures_boost::is_signed<R>::value || geofeatures_boost::is_floating_point<R>::value>());
      *result = negate_integer(*result, mpl::bool_<geofeatures_boost::is_signed<R>::value || geofeatures_boost::is_floating_point<R>::value>());
   }
}
   ...
   eval_lsb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   // Find the index of the least significant bit within that limb:
   //
   return geofeatures_boost::multiprecision::detail::find_lsb(*a.limbs());
}
   ...
   eval_msb(const cpp_int_backend<MinBits1, MaxBits1, SignType1, Checked1, Allocator1>& a)
{
   ...
   // Find the index of the least significant bit within that limb:
   //
   return geofeatures_boost::multiprecision::detail::find_msb(*a.limbs());
}
