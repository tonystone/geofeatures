...
namespace backends{
  ...
  using geofeatures_boost::enable_if;
...
private:
   union data_type
   {
      ...
      BOOST_CONSTEXPR data_type(signed_limb_type i) : first(i < 0 ? static_cast<limb_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(i)) : i) {}
#ifdef BOOST_LITTLE_ENDIAN
      BOOST_CONSTEXPR data_type(double_limb_type i) : double_first(i) {}
      BOOST_CONSTEXPR data_type(signed_double_limb_type i) : double_first(i < 0 ? static_cast<double_limb_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(i)) : i) {}
#endif
   };
...
public:
   ...
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(signed_double_limb_type i)BOOST_NOEXCEPT
      : m_data(i), m_limbs(i < 0 ? (static_cast<double_limb_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(i)) > static_cast<double_limb_type>(max_limb_value) ? 2 : 1) : (i > max_limb_value ? 2 : 1)),
...
private:
   ...
   geofeatures_boost::uint16_t    m_limbs;
   bool               m_sign;
...
public:
   ...
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(signed_double_limb_type i)BOOST_NOEXCEPT
      : m_wrapper(double_limb_type(i < 0 ? static_cast<double_limb_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(i)) : i)),
        m_limbs(i < 0 ? (static_cast<double_limb_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(i)) > max_limb_value ? 2 : 1) : (i > max_limb_value ? 2 : 1)),
        m_sign(i < 0) {}
#endif
#if defined(BOOST_MP_USER_DEFINED_LITERALS)
      template <limb_type...VALUES>
   ...
   {
      m_limbs = static_cast<geofeatures_boost::uint16_t>((std::min)(new_size, internal_limb_count));
...
#endif
//
// Traits classes to figure out a native type with N bits, these vary from geofeatures_boost::uint_t<N> only
// because some platforms have native integer types longer than geofeatures_boost::long_long_type, "really geofeatures_boost::long_long_type" anyone??
//
...
{
   typedef typename geofeatures_boost::uint_t<N>::least type;
};

template <unsigned N>
struct trivial_limb_type : public trivial_limb_type_imp<N, N <= sizeof(geofeatures_boost::long_long_type) * CHAR_BIT> {};
//
...
protected:
   template <class T>
   typename geofeatures_boost::disable_if_c<!geofeatures_boost::is_integral<T>::value || (std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::digits <= (int)MinBits))>::type
      check_in_range(T val, const mpl::int_<checked>&)
   {
      ...
      if(static_cast<common_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(val)) > static_cast<common_type>(limb_mask))
...
public:
   ...
   //
   template <class SI>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(SI i, typename geofeatures_boost::enable_if_c<is_signed<SI>::value && (Checked == unchecked) >::type const* = 0) BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<SI>())))
      : m_data(i < 0 ? static_cast<local_limb_type>(static_cast<typename make_unsigned<SI>::type>(geofeatures_boost::multiprecision::detail::unsigned_abs(i)) & limb_mask) : static_cast<local_limb_type>(i)& limb_mask), m_sign(i < 0) {}
   template <class SI>
   BOOST_MP_FORCEINLINE cpp_int_base(SI i, typename geofeatures_boost::enable_if_c<is_signed<SI>::value && (Checked == checked) >::type const* = 0) BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<SI>())))
      : m_data(i < 0 ? (static_cast<local_limb_type>(static_cast<typename make_unsigned<SI>::type>(geofeatures_boost::multiprecision::detail::unsigned_abs(i)) & limb_mask)) : static_cast<local_limb_type>(i)& limb_mask), m_sign(i < 0) 
   { check_in_range(i); }
   template <class UI>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(UI i, typename geofeatures_boost::enable_if_c<is_unsigned<UI>::value && (Checked == unchecked)>::type const* = 0) BOOST_NOEXCEPT
      : m_data(static_cast<local_limb_type>(i) & limb_mask), m_sign(false) {}
   template <class UI>
   BOOST_MP_FORCEINLINE cpp_int_base(UI i, typename geofeatures_boost::enable_if_c<is_unsigned<UI>::value && (Checked == checked)>::type const* = 0) BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<UI>())))
      : m_data(static_cast<local_limb_type>(i) & limb_mask), m_sign(false) { check_in_range(i); }
   template <class F>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(F i, typename geofeatures_boost::enable_if_c<is_floating_point<F>::value && (Checked == unchecked)>::type const* = 0) BOOST_NOEXCEPT
      : m_data(static_cast<local_limb_type>(std::fabs(i)) & limb_mask), m_sign(i < 0) {}
   template <class F>
   BOOST_MP_FORCEINLINE cpp_int_base(F i, typename geofeatures_boost::enable_if_c<is_floating_point<F>::value && (Checked == checked)>::type const* = 0)
...
protected:
   template <class T>
   typename geofeatures_boost::disable_if_c<std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::digits <= (int)MinBits)>::type
      check_in_range(T val, const mpl::int_<checked>&, const geofeatures_boost::false_type&)
   {
      typedef typename common_type<T, local_limb_type>::type common_type;
   ...
   template <class T>
   void check_in_range(T val, const mpl::int_<checked>&, const geofeatures_boost::true_type&)
   {
      typedef typename common_type<T, local_limb_type>::type common_type;
   ...
   BOOST_MP_FORCEINLINE void check_in_range(T, const mpl::int_<C>&, const geofeatures_boost::integral_constant<bool, B>&) BOOST_NOEXCEPT {}
...
public:
   ...
   //
   template <class SI>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_base(SI i, typename geofeatures_boost::enable_if_c<is_signed<SI>::value && (Checked == unchecked) >::type const* = 0) BOOST_NOEXCEPT
   ...
   template <class F>
   BOOST_MP_FORCEINLINE cpp_int_base(F i, typename geofeatures_boost::enable_if<is_floating_point<F> >::type const* = 0) BOOST_MP_NOEXCEPT_IF((Checked == unchecked))
...
public:
   typedef typename mpl::if_<
      ...
      geofeatures_boost::long_long_type, signed_double_limb_type>,
      mpl::list<signed_limb_type, signed_double_limb_type>
   >::type                                                           signed_types;
   typedef typename mpl::if_<
      trivial_tag,
      mpl::list<unsigned char, unsigned short, unsigned,
      unsigned long, geofeatures_boost::ulong_long_type, double_limb_type>,
   ...
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_backend(const cpp_int_backend& o) BOOST_MP_NOEXCEPT_IF(geofeatures_boost::is_void<Allocator>::value) : base_type(o) {}
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
   ...
   //
   template <class Arg>
   BOOST_MP_FORCEINLINE BOOST_CONSTEXPR cpp_int_backend(Arg i, typename geofeatures_boost::enable_if_c<is_allowed_cpp_int_base_conversion<Arg, base_type>::value >::type const* = 0)BOOST_MP_NOEXCEPT_IF(noexcept(base_type(std::declval<Arg>())))
      : base_type(i) {}
...
private:
   ...
   void do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other, mpl::false_ const&, mpl::true_ const &)
   {
       ...
       *this = static_cast<
            typename geofeatures_boost::multiprecision::detail::canonical<
...
public:
   ...
   cpp_int_backend(
      ...
      typename geofeatures_boost::enable_if_c<is_implicit_cpp_int_conversion<cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>, self_type>::value>::type* = 0)
      : base_type()
   {
      do_assign(
         other,
   ...
   explicit cpp_int_backend(
      const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other,
      typename geofeatures_boost::disable_if_c<is_implicit_cpp_int_conversion<cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>, self_type>::value>::type* = 0)
      : base_type()
   {
      do_assign(
         other,
...
private:
   template <class A>
   typename geofeatures_boost::enable_if<is_unsigned<A> >::type do_assign_arithmetic(A val, const mpl::true_&) 
   ...
   template <class A>
   typename geofeatures_boost::disable_if_c<is_unsigned<A>::value || !is_integral<A>::value >::type do_assign_arithmetic(A val, const mpl::true_&) 
   ...
   {
      this->check_in_range(val);
      *this->limbs() = (val < 0) ? static_cast<typename self_type::local_limb_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(val)) : static_cast<typename self_type::local_limb_type>(val);
      this->sign(val < 0);
      this->normalize();
   }
   template <class A>
   typename geofeatures_boost::enable_if_c< !is_integral<A>::value>::type do_assign_arithmetic(A val, const mpl::true_&)
   {
      this->check_in_range(val);
      *this->limbs() = (val < 0) ? static_cast<typename self_type::local_limb_type>(geofeatures_boost::multiprecision::detail::abs(val)) : static_cast<typename self_type::local_limb_type>(val);
      this->sign(val < 0);
      this->normalize();
   }
   ...
   {
      this->resize(1, 1);
      *this->limbs() = static_cast<limb_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(i));
      this->sign(i < 0);
   }
   ...
   void do_assign_arithmetic(signed_double_limb_type i, const mpl::false_&) BOOST_MP_NOEXCEPT_IF(noexcept(std::declval<cpp_int_backend>().sign(true)))
   {
      ...
      if(i < 0)
         s = true;
      ui = static_cast<double_limb_type>(geofeatures_boost::multiprecision::detail::unsigned_abs(i));
      typename base_type::limb_pointer p = this->limbs();
   ...
   void do_assign_arithmetic(long double a, const mpl::false_&)
   {
      ...
      BOOST_ASSERT(!(geofeatures_boost::math::isinf)(a));
      BOOST_ASSERT(!(geofeatures_boost::math::isnan)(a));
...
private:
   ...
   std::string do_get_string(std::ios_base::fmtflags f, const mpl::false_&)const
   {
      ...
      else
      {
         ...
         if(this->size() == 1)
         {
            result = geofeatures_boost::lexical_cast<std::string>(t.limbs()[0]);
         }
         else
         {
...
public:
   ...
   template <class Arithmetic>
   BOOST_MP_FORCEINLINE typename geofeatures_boost::enable_if<is_arithmetic<Arithmetic>, int>::type compare(Arithmetic i)const
   {
      // braindead version:
      cpp_int_backend t;
      t = i;
...
using geofeatures_boost::multiprecision::backends::cpp_int_backend;
