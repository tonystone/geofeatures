...
typedef geofeatures_boost::tokenizer<geofeatures_boost::char_separator<char> > tokenizer;
...
struct parsing_assigner
{
                             ...
                             std::string const& wkt)
    {
        ...
        catch(geofeatures_boost::bad_lexical_cast const& blc)
        {
...
inline bool one_of(tokenizer::iterator const& it,
                   ...
                   bool& is_present)
{
    if (geofeatures_boost::iequals(*it, value))
    {
        is_present = true;
        return true;
    }
                       ...
                       tokenizer::iterator& end)
{
    ...
    if (it != end && geofeatures_boost::iequals(*it++, geometry_name))
    {
...
struct geometry_parser
{
    static inline void apply(std::string const& wkt, Geometry& geometry)
    {
        ...
        tokenizer tokens(wkt, geofeatures_boost::char_separator<char>(" ", ",()"));
...
struct multi_parser
{
    static inline void apply(std::string const& wkt, MultiGeometry& geometry)
    {
        ...
        if (initialize<MultiGeometry>(tokens, PrefixPolicy::apply(), wkt, it, end))
        {
            ...
            while(it != end && *it != ")")
            {
                traits::resize<MultiGeometry>::apply(geometry, geofeatures_boost::size(geometry) + 1);
                Parser
                    <
                        typename geofeatures_boost::range_value<MultiGeometry>::type
                    >::apply(it, end, wkt, *(geofeatures_boost::end(geometry) - 1));
                if (it != end && *it == ",")
                {
...
struct box_parser
{
    static inline void apply(std::string const& wkt, Box& box)
    {
        ...
        if (it != end && geofeatures_boost::iequals(*it, "POLYGON"))
        {
            ++it;
            bool has_empty, has_z, has_m;
            handle_empty_z_m(it, end, has_empty, has_z, has_m);
        ...
        else if (it != end && geofeatures_boost::iequals(*it, "BOX"))
        {
            ++it;
        }
        else
        ...
        std::size_t n = geofeatures_boost::size(points);
        if (n == 2)
        {
            index = 1;
        }
...
struct segment_parser
{
    static inline void apply(std::string const& wkt, Segment& segment)
    {
        ...
        tokenizer::iterator end = tokens.end();
        if (it != end &&
            (geofeatures_boost::iequals(*it, "SEGMENT")
            || geofeatures_boost::iequals(*it, "LINESTRING") ))
        {
            ++it;
        }
        else
        ...
        if (geofeatures_boost::size(points) == 2)
        {
