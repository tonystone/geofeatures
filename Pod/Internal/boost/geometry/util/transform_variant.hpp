...
/*!
    ...
    \par Example
    \code
        typedef geofeatures_boost::mpl::vector<int, float, long> types;
    ...
    \endcode
*/
template <typename Sequence, typename Op, typename In = geofeatures_boost::mpl::na>
struct transform_variant:
    make_variant_over<
        typename geofeatures_boost::mpl::transform<
            Sequence,
            Op,
            In
        >::type
...
/*!
    \brief Meta-function that takes a geofeatures_boost::variant type and an MPL lambda
...
struct transform_variant<variant<BOOST_VARIANT_ENUM_PARAMS(T)>, Op, geofeatures_boost::mpl::na> :
    make_variant_over<
        typename geofeatures_boost::mpl::transform<
