...
template <typename T,
          bool IsFloatingPoint = geofeatures_boost::is_floating_point<T>::value>
struct abs
{
...
template <typename Type,
          bool IsFloatingPoint = geofeatures_boost::is_floating_point<Type>::value>
struct equals
{
...
struct equals<Type, true>
{
    ...
    {
        geofeatures_boost::ignore_unused(policy);
        ...
        if (geofeatures_boost::math::isfinite(a) && geofeatures_boost::math::isfinite(b))
        {
...
<
    typename T,
    bool IsFundemantal = geofeatures_boost::is_fundamental<T>::value /* false */
>
struct square_root
{
...
struct square_root_for_fundamental_fp
{
    ...
    static inline FundamentalFP apply(FundamentalFP const& value)
    {
        ...
        if (geofeatures_boost::math::isfinite(value))
        {
            return std::sqrt(value);
        }
        else if (geofeatures_boost::math::isinf(value) && value < 0)
        {
...
struct square_root<T, true>
{
    ...
    static inline double apply(T const& value)
    {
        ...
        return square_root_for_fundamental_fp
            <
                double
            >::apply(geofeatures_boost::numeric_cast<double>(value));
    }
};
...
<
    ...
    bool IsIntegral = geofeatures_boost::is_integral<Fundamental>::value
>
...
struct define_pi
{
    static inline T apply()
    {
        ...
        return geofeatures_boost::math::constants::pi<T>();
    }
};
...
struct define_two_pi
{
    ...
    {
        // Default calls Boost.Math
        return geofeatures_boost::math::constants::two_pi<T>();
    }
};
...
struct define_half_pi
{
    ...
    {
        // Default calls Boost.Math
        return geofeatures_boost::math::constants::half_pi<T>();
    }
};
...
struct rounding_cast
{
    ...
    {
        return geofeatures_boost::numeric_cast<Result>(v);
    }
};
...
struct rounding_cast<Result, Source, true, false>
{
    static inline Result apply(Source const& v)
    {
        return geofeatures_boost::numeric_cast<Result>(v < Source(0) ?
                                            v - Source(0.5) :
                                            v + Source(0.5));
    }
};
...
sqrt(T const& value)
{
    return detail::square_root
        <
            T, geofeatures_boost::is_fundamental<T>::value
        >::apply(value);
}
