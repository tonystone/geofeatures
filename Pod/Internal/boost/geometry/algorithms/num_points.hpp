...
struct num_points<Geometry, AddForOpen, multi_tag>
    : detail::counting::multi_count
        <
            num_points<typename geofeatures_boost::range_value<Geometry>::type, AddForOpen>
        >
{};
...
struct num_points<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    struct visitor: geofeatures_boost::static_visitor<std::size_t>
    {
        bool m_add_for_open;
    ...
    apply(geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const& geometry,
          bool add_for_open)
    {
        return geofeatures_boost::apply_visitor(visitor(add_for_open), geometry);
    }
};
