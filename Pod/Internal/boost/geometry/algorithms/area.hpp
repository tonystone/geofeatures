...
struct ring_area
{
    ...
    apply(Ring const& ring, Strategy const& strategy)
    {
        ...
        if (geofeatures_boost::size(ring)
...
struct area<MultiGeometry, multi_polygon_tag> : detail::multi_sum
{
    ...
    apply(MultiGeometry const& multi, Strategy const& strategy)
    {
        return multi_sum::apply
               <
                   typename Strategy::return_type,
                   area<typename geofeatures_boost::range_value<MultiGeometry>::type>
               >(multi, strategy);
    }
};
...
struct area<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    template <typename Strategy>
    struct visitor: geofeatures_boost::static_visitor<typename Strategy::return_type>
    {
