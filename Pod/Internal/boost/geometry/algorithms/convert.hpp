...
struct point_to_box
{
    static inline void apply(Point const& point, Box& box)
    {
        ...
        set<Index, Dimension>(box,
                geofeatures_boost::numeric_cast<coordinate_type>(get<Dimension>(point)));
        point_to_box
            <
                Point, Box,
...
struct segment_to_range
{
    static inline void apply(Segment const& segment, Range& range)
    {
        ...
        typename geofeatures_boost::range_iterator<Range>::type it = geofeatures_boost::begin(range);
...
struct range_to_range
{
    ...
    static inline void apply(Range1 const& source, Range2& destination)
    {
        ...
        typedef typename geofeatures_boost::range_size<Range1>::type size_type;
        size_type n = geofeatures_boost::size(view);
        ...
        size_type i = 0;
        for (typename geofeatures_boost::range_iterator<view_type const>::type it
            = geofeatures_boost::begin(view);
            it != geofeatures_boost::end(view) && i < n;
            ++it, ++i)
        {
...
struct polygon_to_polygon
{
    ...
    static inline void apply(Polygon1 const& source, Polygon2& destination)
    {
        ...
        traits::resize
            <
                typename geofeatures_boost::remove_reference
                <
        ...
        typename detail::interior_iterator<Polygon1 const>::type
            it_source = geofeatures_boost::begin(rings_source);
        typename detail::interior_iterator<Polygon2>::type
            it_dest = geofeatures_boost::begin(rings_dest);

        for ( ; it_source != geofeatures_boost::end(rings_source); ++it_source, ++it_dest)
        {
...
struct single_to_multi: private Policy
{
    static inline void apply(Single const& single, Multi& multi)
    {
        ...
        Policy::apply(single, *geofeatures_boost::begin(multi));
    }
};
...
struct multi_to_multi: private Policy
{
    ...
    {
        traits::resize<Multi2>::apply(multi2, geofeatures_boost::size(multi1));

        typename geofeatures_boost::range_iterator<Multi1 const>::type it1
                = geofeatures_boost::begin(multi1);
        typename geofeatures_boost::range_iterator<Multi2>::type it2
                = geofeatures_boost::begin(multi2);

        for (; it1 != geofeatures_boost::end(multi1); ++it1, ++it2)
        {
...
<
    ...
    bool UseAssignment = geofeatures_boost::is_same<Geometry1, Geometry2>::value
                         && !geofeatures_boost::is_array<Geometry1>::value
>
struct convert: not_implemented<Tag1, Tag2, geofeatures_boost::mpl::int_<DimensionCount> >
{};
...
// version types might be the same and therefore we call geofeatures_boost::is_same again
...
struct convert<Multi1, Multi2, multi_tag, multi_tag, DimensionCount, false>
    : detail::conversion::multi_to_multi
        <
            ...
            convert
                <
                    typename geofeatures_boost::range_value<Multi1>::type,
                    typename geofeatures_boost::range_value<Multi2>::type,
    ...
    : detail::conversion::single_to_multi
        <
            ...
            convert
                <
                    Single,
                    typename geofeatures_boost::range_value<Multi>::type,
...
struct convert<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>, Geometry2>
{
    ...
    static inline void apply(
        geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const& geometry1,
        Geometry2& geometry2
    )
    {
        geofeatures_boost::apply_visitor(visitor(geometry2), geometry1);
    }
};
