...
{
    typedef typename geofeatures_boost::mpl::at_c<DimensionVector, Index>::type dimension;
...
struct sectionalize_part
{
    static const std::size_t dimension_count
        = geofeatures_boost::mpl::size<DimensionVector>::value;
    ...
    {
        geofeatures_boost::ignore_unused_variable_warning(robust_policy);

        typedef typename geofeatures_boost::range_value<Sections>::type section_type;
        BOOST_STATIC_ASSERT
            (
                (static_cast<std::size_t>(section_type::dimension_count)
                 == static_cast<std::size_t>(geofeatures_boost::mpl::size<DimensionVector>::value))
            );
...
struct sectionalize_range
{
                             ...
                             std::size_t max_count)
    {
        ...
        std::size_t const n = geofeatures_boost::size(view);
        if (n == 0)
        {
        ...
        sectionalize_part<Point, DimensionVector>::apply(sections,
            geofeatures_boost::begin(view), geofeatures_boost::end(view),
...
struct sectionalize_polygon
{
                ...
                ring_identifier ring_id, std::size_t max_count)
    {
        ...
        for (typename detail::interior_iterator<Polygon const>::type
                it = geofeatures_boost::begin(rings); it != geofeatures_boost::end(rings); ++it, ++ring_id.ring_index)
        {
...
inline void enlarge_sections(Sections& sections)
{
    ...
    for (typename geofeatures_boost::range_iterator<Sections>::type it = geofeatures_boost::begin(sections);
        it != geofeatures_boost::end(sections);
        ++it)
    {
        typedef typename geofeatures_boost::range_value<Sections>::type section_type;
                ...
                std::size_t max_count = 10)
{
    ...
    BOOST_MPL_ASSERT((geofeatures_boost::is_same<ctype1, ctype2>));
