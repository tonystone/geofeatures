...
int point_in_range(Point const& point, Range const& range, Strategy const& strategy)
{
    ...
    iterator_type it = geofeatures_boost::begin(range);
    iterator_type end = geofeatures_boost::end(range);
...
inline int point_in_range(Point const& point, Range const& range)
{
    ...
    static const bool same_strategies = geofeatures_boost::is_same<strategy_type, strategy_type2>::value;
...
struct point_in_geometry<Linestring, linestring_tag>
{
    ...
    {
        std::size_t count = geofeatures_boost::size(linestring);
        if ( count > 1 )
        {
...
struct point_in_geometry<Ring, ring_tag>
{
    ...
    {
        if ( geofeatures_boost::size(ring) < core_detail::closure::minimum_ring_size
                                    <
                            ...
                            Strategy const& strategy)
    {
        ...
        if (code == 1)
        {
            ...
            for (typename detail::interior_iterator<Polygon const>::type
                 it = geofeatures_boost::begin(rings);
                 it != geofeatures_boost::end(rings);
                 ++it)
            {
...
struct point_in_geometry<Geometry, multi_point_tag>
{
    ...
    {
        typedef typename geofeatures_boost::range_value<Geometry>::type point_type;
        typedef typename geofeatures_boost::range_const_iterator<Geometry>::type iterator;
        for ( iterator it = geofeatures_boost::begin(geometry) ; it != geofeatures_boost::end(geometry) ; ++it )
        {
...
struct point_in_geometry<Geometry, multi_linestring_tag>
{
    ...
    int apply(Point const& point, Geometry const& geometry, Strategy const& strategy)
    {
        ...
        typedef typename geofeatures_boost::range_value<Geometry>::type linestring_type;
        typedef typename geofeatures_boost::range_value<linestring_type>::type point_type;
        typedef typename geofeatures_boost::range_iterator<Geometry const>::type iterator;
        iterator it = geofeatures_boost::begin(geometry);
        for ( ; it != geofeatures_boost::end(geometry) ; ++it )
        {
        ...
        for ( ; it != geofeatures_boost::end(geometry) ; ++it )
        {
            if ( geofeatures_boost::size(*it) < 2 )
                continue;
