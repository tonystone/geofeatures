...
struct point_on_range
{
    ...
    {
        const std::size_t n = geofeatures_boost::size(range);
        if (midpoint && n > 1)
        {
            ...
            if (it != geofeatures_boost::end(range))
            {
        ...
        if (n > 0)
        {
            geometry::detail::conversion::convert_point_to_point(*geofeatures_boost::begin(range), point);
            return true;
        }
        return false;
    }
...
struct point_on_multi
{
    static inline bool apply(Point& point, MultiGeometry const& multi, bool midpoint)
    {
        ...
        for (typename geofeatures_boost::range_iterator
                <
                    MultiGeometry const
                >::type it = geofeatures_boost::begin(multi);
