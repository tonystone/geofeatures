...
struct get_ring<void>
{
    ...
    static inline typename geofeatures_boost::range_value<Range>::type const&
...
struct get_ring<polygon_tag>
{
                ...
                Polygon const& polygon)
    {
        BOOST_GEOMETRY_ASSERT
            (
                id.ring_index >= -1
                && id.ring_index < int(geofeatures_boost::size(interior_rings(polygon)))
            );
...
struct get_ring<multi_polygon_tag>
{
                ...
                MultiPolygon const& multi_polygon)
    {
        BOOST_GEOMETRY_ASSERT
            (
                id.multi_index >= 0
                && id.multi_index < int(geofeatures_boost::size(multi_polygon))
            );
