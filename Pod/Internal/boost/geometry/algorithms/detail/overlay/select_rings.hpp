...
namespace dispatch
{
    ...
    struct select_rings<ring_tag, Ring>
    {
        ...
        static inline void apply(Ring const& ring, Geometry const& ,
                    ring_identifier const& id, RingPropertyMap& ring_properties)
        {
            if (geofeatures_boost::size(ring) > 0)
            {
    ...
    struct select_rings<multi_polygon_tag, Multi>
    {
                    ...
                    ring_identifier id, RingPropertyMap& ring_properties)
        {
            ...
            typedef select_rings<polygon_tag, typename geofeatures_boost::range_value<Multi>::type> per_polygon;

            id.multi_index = 0;
            for (iterator_type it = geofeatures_boost::begin(multi); it != geofeatures_boost::end(multi); ++it)
            {
                id.ring_index = -1;
            ...
            RingPropertyMap& selected_ring_properties)
{
    ...
    for (typename RingPropertyMap::const_iterator it = geofeatures_boost::begin(all_ring_properties);
        it != geofeatures_boost::end(all_ring_properties);
        ++it)
    {
