...
public:
    ...
    sweep_event(Interval const& interval, bool start_event = true)
        : m_interval(geofeatures_boost::cref(interval))
...
private:
    geofeatures_boost::reference_wrapper<Interval const> m_interval;
    bool m_start_event;
};
...
struct initialization_visitor
{
                             ...
                             EventVisitor&)
    {
        ...
        PriorityQueue pq(geofeatures_boost::begin(range), geofeatures_boost::end(range));
        std::swap(pq, queue);
    }
};
...
inline typename geofeatures_boost::range_value<RangeOfIntervals>::type::difference_type
maximum_gap(RangeOfIntervals const& range_of_intervals,
            T& max_gap_left, T& max_gap_right)
{
    typedef typename geofeatures_boost::range_value<RangeOfIntervals>::type interval_type;
