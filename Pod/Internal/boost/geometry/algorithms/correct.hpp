...
struct correct_ring
{
    ...
    {
        // Check close-ness
        if (geofeatures_boost::size(r) > 2)
        {
            // check if closed, if not, close it
            bool const disjoint = geometry::disjoint(*geofeatures_boost::begin(r), *(geofeatures_boost::end(r) - 1));
            closure_selector const s = geometry::closure<Ring>::value;

            if (disjoint && (s == closed))
            {
                geometry::append(r, *geofeatures_boost::begin(r));
            }
            if (! disjoint && s != closed)
            {
                // Open it by removing last point
                geometry::traits::resize<Ring>::apply(r, geofeatures_boost::size(r) - 1);
            }
        }
        // Check area
        Predicate predicate;
        ...
        {
            std::reverse(geofeatures_boost::begin(r), geofeatures_boost::end(r));
        }
    }
};
...
struct correct<geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    ...
    static inline void
    apply(geofeatures_boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>& geometry)
    {
