...
struct fe_range_per_point
{
    ...
    static inline void apply(Range& range, Functor& f)
    {
        ...
        // return (std::for_each(geofeatures_boost::begin(range), geofeatures_boost::end(range), f));
        ...
        for (typename geofeatures_boost::range_iterator<Range>::type
                it = geofeatures_boost::begin(range); it != geofeatures_boost::end(range); ++it)
        {
            f(*it);
        }
    }
...
struct fe_range_per_segment_with_closure
{
    ...
    static inline void apply(Range& range, Functor& f)
    {
        ...
        while(it != geofeatures_boost::end(range))
        {
...
struct for_each_multi
{
    ...
    static inline void apply(MultiGeometry& multi, Functor& f)
    {
        for (typename geofeatures_boost::range_iterator<MultiGeometry>::type
                it = geofeatures_boost::begin(multi); it != geofeatures_boost::end(multi); ++it)
        {
            Policy::apply(*it, f);
        }
    }
