...
- there must be a specialization of traits::dimension defining its number
  ...
  from geofeatures_boost::mpl::int_&lt;X&gt; for X-D)
...
- for non-Cartesian coordinate systems, the coordinate system's units
  must either be geofeatures_boost::geometry::degree or geofeatures_boost::geometry::radian
...
class ConstPoint
{
    ...
    struct dimension_checker
    {
        static void apply()
        {
            ...
            geofeatures_boost::ignore_unused(p, coord);
            dimension_checker<P, Dimension+1, DimensionCount>::apply();
        }
    };
