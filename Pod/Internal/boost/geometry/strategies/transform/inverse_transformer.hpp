...
public :
    ...
    {
        typedef geofeatures_boost::numeric::ublas::matrix<CalculationType> matrix_type;

        // create a working copy of the input
        matrix_type copy(input.matrix());

        // create a permutation matrix for the LU-factorization
        typedef geofeatures_boost::numeric::ublas::permutation_matrix<> permutation_matrix;
        permutation_matrix pm(copy.size1());

        // perform LU-factorization
        int res = geofeatures_boost::numeric::ublas::lu_factorize<matrix_type>(copy, pm);
        if( res == 0 )
        {
            // create identity matrix
            this->m_matrix.assign(geofeatures_boost::numeric::ublas::identity_matrix<CalculationType>(copy.size1()));

            // backsubstitute to get the inverse
            geofeatures_boost::numeric::ublas::lu_substitute(copy, pm, this->m_matrix);
        }
    }
