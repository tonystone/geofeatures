...
#include <boost/move/utility_core.hpp> //for geofeatures_boost::move

#if !defined(BOOST_MOVE_DOXYGEN_INVOKED)

#if defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
namespace geofeatures_boost_move_member_swap {
...
namespace geofeatures_boost_move_adl_swap{
...
template<class T>
void swap_proxy(T& x, T& y, typename geofeatures_boost::move_detail::enable_if_c<!geofeatures_boost::move_detail::has_move_emulation_enabled_impl<T>::value>::type* = 0)
{
...
void swap_proxy(T& x, T& y
               , typename geofeatures_boost::move_detail::enable_if< and_op_not_impl<geofeatures_boost::move_detail::has_move_emulation_enabled_impl<T>
                                                                        , geofeatures_boost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  T t(::geofeatures_boost::move(x)); x = ::geofeatures_boost::move(y); y = ::geofeatures_boost::move(t);  }

template<class T>
void swap_proxy(T& x, T& y
               , typename geofeatures_boost::move_detail::enable_if< and_op_impl< geofeatures_boost::move_detail::has_move_emulation_enabled_impl<T>
                                                                    , geofeatures_boost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  x.swap(y);  }
...
void swap_proxy(T (& x)[N], T (& y)[N])
{
   for (std::size_t i = 0; i < N; ++i){
      ::geofeatures_boost_move_adl_swap::swap_proxy(x[i], y[i]);
   }
}
...
//!      <code>T t(::geofeatures_boost::move(x)); x = ::geofeatures_boost::move(y); y = ::geofeatures_boost::move(t);</code>.
template<class T>
void adl_move_swap(T& x, T& y)
{
   ::geofeatures_boost_move_adl_swap::swap_proxy(x, y);
}
