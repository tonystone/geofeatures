...
#include <boost/type_traits/is_floating_point.hpp> // for geofeatures_boost::is_floating_point;
#include <boost/type_traits/is_integral.hpp> // for geofeatures_boost::is_integral
#include <boost/type_traits/is_convertible.hpp> // for geofeatures_boost::is_convertible
#include <boost/type_traits/is_same.hpp>// for geofeatures_boost::is_same
#include <boost/type_traits/remove_cv.hpp>// for geofeatures_boost::remove_cv
// Boost Template meta programming:
#include <boost/mpl/if.hpp> // for geofeatures_boost::mpl::if_c.
#include <boost/mpl/and.hpp> // for geofeatures_boost::mpl::if_c.
#include <boost/mpl/or.hpp> // for geofeatures_boost::mpl::if_c.
#include <boost/mpl/not.hpp> // for geofeatures_boost::mpl::if_c.
    ...
    namespace tools
    {
      ...
      { // Promote, if necessary, & pick the wider of the two floating-point types.
        ...
        typedef typename mpl::if_<
          ...
          typename mpl::if_< typename mpl::and_<mpl::not_<is_floating_point<T2P> >, ::geofeatures_boost::is_convertible<T1P, T2P> >, T2P, T1P>::type>::type type;
      }; // promote_arg2
      // These full specialisations reduce mpl::if_ usage and speed up
      // compilation:
      ...
      struct promote_args
      {
         ...
         //
         BOOST_STATIC_ASSERT_MSG((0 == ::geofeatures_boost::is_same<type, long double>::value), "Sorry, but this platform does not have sufficient long double support for the special functions to be reliably implemented.");
#endif
      };
