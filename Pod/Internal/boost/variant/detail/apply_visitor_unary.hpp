...
struct result_multideduce1 {
    ...
    typedef typename geofeatures_boost::mpl::begin<types>::type begin_it;
    typedef typename geofeatures_boost::mpl::advance<
        begin_it, geofeatures_boost::mpl::int_<geofeatures_boost::mpl::size<types>::type::value - 1>
    >::type                                         last_it;
    ...
    static Visitor& vis() BOOST_NOEXCEPT {
        ...
        // Because of that, we can not use `geofeatures_boost::decval<Visitor&>()` here.
        Visitor&(*f)() = 0; // pointer to function
        return f();
    }

    static decltype(auto) deduce_impl(last_it, unsigned /*helper*/) {
        typedef typename geofeatures_boost::mpl::deref<last_it>::type value_t;
        return vis()( geofeatures_boost::declval< value_t& >() );
    }

    template <class It>
    static decltype(auto) deduce_impl(It, unsigned helper) {
        typedef typename geofeatures_boost::mpl::next<It>::type next_t;
        typedef typename geofeatures_boost::mpl::deref<It>::type value_t;
        if (helper == geofeatures_boost::mpl::distance<begin_it, It>::type::value) {
...
inline decltype(auto) apply_visitor(Visitor& visitor, Visitable& visitable,
    typename geofeatures_boost::disable_if<
        geofeatures_boost::detail::variant::has_result_type<Visitor>
    >::type* = 0)
{
    geofeatures_boost::detail::variant::result_wrapper1<Visitor, Visitable> cpp14_vis(visitor);
...
inline decltype(auto) apply_visitor(const Visitor& visitor, Visitable& visitable,
    ...
    >::type* = 0)
{
    geofeatures_boost::detail::variant::result_wrapper1<const Visitor, Visitable> cpp14_vis(visitor);
    return visitable.apply_visitor(cpp14_vis);
}
