...
namespace geofeatures_boost {} namespace boost = geofeatures_boost; namespace geofeatures_boost {
    ...
    namespace detail { namespace variant {
        struct variant_hasher: public geofeatures_boost::static_visitor<std::size_t> {
            template <class T>
    ...
    std::size_t hash_value(variant< BOOST_VARIANT_ENUM_PARAMS(T) > const& val) {
        std::size_t seed = geofeatures_boost::apply_visitor(detail::variant::variant_hasher(), val);
