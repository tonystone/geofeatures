...
private:
    ...
    template<class C>
    static geofeatures_boost::uint8_t f(check<std::size_t(C::*)(void) const, &C::size>*);

    template<class C>
    static geofeatures_boost::uint16_t f(...);

public:
    static const bool value =
        (sizeof(f<T>(0)) == sizeof(geofeatures_boost::uint8_t));
